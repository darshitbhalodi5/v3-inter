import { Token as SDKToken } from "darshitswap-sdk-core";
import {
  Currency,
  HistoryDuration,
  PriceSource,
...
export const token = createFixture<Token, TokenOptions>({ sdkToken: null })(
  ({ sdkToken }) => ({
    __typename: "Token",
    ...
    chain:
      (sdkToken ? toGraphQLChain(sdkToken.chainId) : null) ??
      randomChoice(GQL_CHAINS),
    address:
      sdkToken?.address.toLocaleLowerCase() ?? faker.finance.ethereumAddress(),
    market: undefined,
...
export const ethToken = createFixture<Token>()(() =>
  token({ sdkToken: ETH, project: ethProject })
);
...
export const daiToken = createFixture<Token>()(() =>
  token({ sdkToken: DAI, project: daiProject })
);
