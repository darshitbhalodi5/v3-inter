...
describe(tokenProjectToCurrencyInfos, () => {
  ...
  it("converts tokenProject to CurrencyInfo", () => {
    ...
    expect(result).toEqual(
      project.tokens.map((token) => getExpectedResult(project, token))
    );
  });
  ...
  it("filters out values for which currency is invalid", () => {
    ...
    const result = tokenProjectToCurrencyInfos(
      [projectWithInvalidTokens],
      ChainId.Mainnet
    );

    expect(result).toEqual([
      getExpectedResult(project, project.tokens[0] as GQLToken),
    ]);
  });
});
...
describe(gqlTokenToCurrencyInfo, () => {
  ...
  it("returns null if currency is invalid", () => {
    const result = gqlTokenToCurrencyInfo(
      ethToken({ chain: "INVALID" as Chain })
    );
