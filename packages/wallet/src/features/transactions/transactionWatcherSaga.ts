...
import { TradeType } from "darshitswap-sdk-core";
import {
  FeatureFlags,
  getFeatureFlagName,
  ...
  for (const transaction of incompleteTransactions) {
    if (transaction.typeInfo.type === TransactionType.FiatPurchase) {
      yield* fork(
        ...
        transaction as FiatOnRampTransactionDetails
      );
    } else {
...
export function* watchFiatOnRampTransaction(
  transaction: FiatOnRampTransactionDetails
) {
  ...
  try {
    while (true) {
      ...
      if (JSON.stringify(updatedTransaction) !== JSON.stringify(transaction)) {
        ...
        const isTransfer =
          updatedTransaction.typeInfo.inputSymbol ===
          updatedTransaction.typeInfo.outputSymbol;
...
function* waitForCancellation(chainId: ChainId, id: string) {
  while (true) {
    const { payload } = yield* take<ReturnType<typeof cancelTransaction>>(
...
function* waitForReplacement(chainId: ChainId, id: string) {
  while (true) {
    const { payload } = yield* take<ReturnType<typeof replaceTransaction>>(
      replaceTransaction.type
    );
...
export function logTransactionEvent(
  ...
  if (type === TransactionType.Send) {
    const { tokenAddress, recipient: toAddress } =
      typeInfo as SendTokenTransactionInfo;
...
function* finalizeTransaction({
  ...
  const status =
    statusOverride ??
    getFinalizedTransactionStatus(transaction.status, ethersReceipt?.status);
  ...
  yield* put(
    setNotificationStatus({ address: transaction.from, hasNotifications: true })
  );
