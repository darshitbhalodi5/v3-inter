...
export function FeeOnTransferFeeGroup({
  ...
  return (
    <Flex gap="$spacing12">
      {inputTokenInfo.fee.greaterThan(0) && (
        <FeeOnTransferFeeRow feeInfo={inputTokenInfo} />
      )}
      {outputTokenInfo.fee.greaterThan(0) && (
        <FeeOnTransferFeeRow feeInfo={outputTokenInfo} />
      )}
    </Flex>
  );
}

function FeeOnTransferFeeRow({
  feeInfo,
}: {
    ...
    <Flex row alignItems="center" justifyContent="space-between">
      <FeeOnTransferWarning>
        <Text color="$neutral2" variant="body3">
          {t("swap.details.feeOnTransfer", {
            tokenSymbol: feeInfo.tokenSymbol,
          })}
        </Text>
      </FeeOnTransferWarning>
