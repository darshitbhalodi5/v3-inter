...
export function getBaseTradeAnalyticsProperties({
  ...
  return {
    ...
    token_in_address: getCurrencyAddressForAnalytics(
      trade.inputAmount.currency
    ),
    token_out_address: getCurrencyAddressForAnalytics(
...
export function getBaseTradeAnalyticsPropertiesFromSwapInfo({
  ...
  const slippageTolerance =
    ...
    derivedSwapInfo.autoSlippageTolerance;

  const portionAmount = getPortionAmountFromQuoteData(
    derivedSwapInfo.trade.trade?.quoteData
  );
  ...
  return {
    ...
    allowed_slippage_basis_points: slippageTolerance
...
function getPortionAmountFromQuoteData(
  quoteData?: QuoteData
): string | undefined {
  if (!quoteData?.quote) {
    return undefined;
