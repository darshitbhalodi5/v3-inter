...
import { Currency, TradeType } from "darshitswap-sdk-core";
...
const getFeeAmountUsd = (
  ...
  const feeUSD =
    parseFloat(outputCurrencyPricePerUnitExact) *
    parseFloat(currencyAmount.toExact());
  return feeUSD;
};
...
export function SwapDetails({
  ...
  const swapFeeFiatFormatted = convertFiatAmountFormatted(
    swapFeeUsd,
...
function calculatePercentageDifference({
  ...
  const newAmount =
    derivedSwapInfo.currencyAmounts[derivedCurrencyField]?.asFraction;
  const acceptedAmount =
    acceptedDerivedSwapInfo.currencyAmounts[derivedCurrencyField]?.asFraction;

  if (!newAmount || !acceptedAmount) {
    return null;
  }

  const percentage = newAmount
    .subtract(acceptedAmount)
    .divide(acceptedAmount)
    .multiply(100);
