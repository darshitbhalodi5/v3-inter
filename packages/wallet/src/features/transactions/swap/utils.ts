...
import { Currency, Percent, TradeType } from "darshitswap-sdk-core";
import { Protocol } from "darshitswap-router-sdk";
import {
  FlatFeeOptions,
  SwapOptions as UniversalRouterSwapOptions,
  SwapRouter as UniversalSwapRouter,
} from "darshitswap-universal-router-sdk";
import { FeeOptions } from "darshitswap-v3-sdk";
...
export function isWrapAction(
  wrapType: WrapType
): wrapType is WrapType.Unwrap | WrapType.Wrap {
...
export function tradeToTransactionInfo(
  ...
  const slippageTolerancePercent = slippageToleranceToPercent(
    trade.slippageTolerance
  );
  const { quoteData, slippageTolerance } = trade;

  const { quoteId, gasUseEstimate, routeString } =
    parseQuoteTypeSpecificParms(quoteData);
...
function parseQuoteTypeSpecificParms(quoteData: QuoteData | undefined): {
  ...
  const quoteId = isLegacyQuote
    ? quoteData?.quote?.quoteId
    : maybeTradingApiQuote?.quoteId;
...
export function requireAcceptNewTrade(
  oldTrade: Maybe<Trade>,
  newTrade: Maybe<Trade>
): boolean {
  ...
  return (
    ...
    newTrade.executionPrice.lessThan(
      oldTrade.worstExecutionPrice(ACCEPT_NEW_TRADE_THRESHOLD)
    )
  );
}
...
export const getRateToDisplay = (
  ...
  const quoteCurrencySymbol = getSymbolDisplayText(
    trade.executionPrice.quoteCurrency.symbol
  );
  const baseCurrencySymbol = getSymbolDisplayText(
...
export function sumGasFees(
  gasFee1?: string | undefined,
  gasFee2?: string
): string | undefined {
  if (!gasFee1 || !gasFee2) {
