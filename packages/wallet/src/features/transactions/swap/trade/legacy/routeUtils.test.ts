...
const amount = (raw: TemplateStringsArray): string =>
  BigNumber.from(raw[0]).mul(1e6).toString();
...
describe("#useRoute", () => {
  ...
  it("handles a multi-route trade from DAI to USDC", () => {
    ...
    expect(result?.[1]?.routev3?.tokenPath).toEqual([
      DAI_TOKEN,
      MKR_TOKEN,
      USDC_TOKEN,
    ]);
  ...
  describe("with ETH", () => {
    it("outputs native ETH as input currency", () => {
      ...
      expect(result?.[0]?.routev3?.output).toStrictEqual(USDC_TOKEN);
      expect(result?.[0]?.routev3?.tokenPath).toStrictEqual([
        WETH_TOKEN,
        USDC_TOKEN,
      ]);
    ...
    it("outputs native ETH as input currency for v2 routes", () => {
      ...
      expect(result?.[0]?.routev2?.output).toStrictEqual(USDC_TOKEN);
      expect(result?.[0]?.routev2?.path).toStrictEqual([
        WETH_TOKEN,
        USDC_TOKEN,
      ]);
      expect(result?.[0]?.outputAmount.toSignificant()).toBe("5");
