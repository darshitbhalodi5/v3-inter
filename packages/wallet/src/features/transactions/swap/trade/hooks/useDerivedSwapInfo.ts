...
export function useDerivedSwapInfo(state: TransactionState): DerivedSwapInfo {
  ...
  const currencyInInfo = useCurrencyInfo(
    currencyAssetIn
      ? buildCurrencyId(currencyAssetIn.chainId, currencyAssetIn.address)
      : undefined
  );
  ...
  const chainId =
    currencyIn?.chainId ?? currencyOut?.chainId ?? ChainId.Mainnet;
  ...
  const inputCurrencyUSDValue = useUSDCValue(
    currencyAmounts[CurrencyField.INPUT]
  );
  const outputCurrencyUSDValue = useUSDCValue(
