...
import { MixedRouteSDK } from "darshitswap-router-sdk";
...
const NATIVE_ADDRESS_FOR_TRADING_API =
...
export function transformTradingApiResponseToTrade(
  ...
  const routes = computeRoutesTradingApi(
    tokenInIsNative,
    tokenOutIsNative,
    data
  );
...
export function computeRoutesTradingApi(
  ...
  if (
    !quoteResponse ||
    !quoteResponse.quote ||
    !isClassicQuote(quoteResponse.quote)
  ) {
    return;
  }
...
function parseTokenApi(token: TradingApiTokenInRoute): Token {
  ...
  if (!chainId || !address || !decimals || !symbol) {
    throw new Error(
      "Expected token to have chainId, address, decimals, and symbol"
    );
  }
  return new Token(
    chainId,
...
export function getTokenAddressForApiRequest(
...
const SUPPORTED_TRADING_API_CHAIN_IDS: number[] = Object.values(
  TradingApiChainId
).filter((value): value is number => typeof value === "number");

// Parse any chain id to check if its supported by the API ChainId type
function isTradingApiSupportedChainId(
  chainId?: number
): chainId is TradingApiChainId {
  if (!chainId) {
    return false;
  }
...
export function getClassicQuoteFromResponse(
  ...
  return isClassicQuote(quoteData.quote?.quote)
    ...
    logger.error(
      ...
      ),
      {
        ...
        extra: {
          ...
          tradeProperties: getBaseTradeAnalyticsProperties({
            trade,
            formatter,
          }),
        },
