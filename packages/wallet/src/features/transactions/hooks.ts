...
export function useSelectTransaction(
  ...
  return useAppSelector((state) =>
    selectTransaction(state, { address, chainId, txId })
  );
}
    ...
    return deDupedTxs.sort((a, b) => {
      ...
      if (a.addedTime === b.addedTime) {
        if (
          ...
          const aCurrencyId = buildCurrencyId(
            a.chainId,
            a.typeInfo.tokenAddress
          );
        ...
        if (
          ...
          const bCurrencyId = buildCurrencyId(
            b.chainId,
            b.typeInfo.tokenAddress
          );
          if (areCurrencyIdsEqual(aCurrencyId, bCurrencyId)) {
            return -1;
          }
