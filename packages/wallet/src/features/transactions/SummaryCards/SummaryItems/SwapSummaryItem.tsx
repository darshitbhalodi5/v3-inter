...
export function SwapSummaryItem({
  ...
  const caption = useMemo(() => {
    ...
    const { inputCurrencyAmountRaw, outputCurrencyAmountRaw } =
      getAmountsFromTrade(typeInfo);
    ...
    const currencyAmount = getFormattedCurrencyAmount(
      ...
      isConfirmedSwapTypeInfo(typeInfo)
        ? false
  ...
  const latestSwapTx = swapCallbacks?.useLatestSwapTransaction(
    transaction.from
  );
  ...
  const shouldShowRetry =
    isTheLatestSwap ||
    (Date.now() - transaction.addedTime < MAX_SHOW_RETRY_TIME &&
      swapCallbacks?.onRetryGenerator);

  const onRetry = swapCallbacks?.onRetryGenerator?.(swapFormState);

  return createElement(
    layoutElement as React.FunctionComponent<TransactionSummaryLayoutProps>,
    {
      caption,
      icon: (
        <SplitLogo
