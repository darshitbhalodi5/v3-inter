...
export function getSwapWarnings(
  ...
  const { currencyBalances, currencyAmounts, currencies, trade } =
    derivedSwapInfo;
  ...
  if (trade.error) {
    ...
    if (
      errorData?.data?.errorCode === SWAP_QUOTE_ERROR ||
      errorData?.message === NO_QUOTE_DATA
    ) {
      warnings.push({
  ...
  if (priceImpact?.greaterThan(PRICE_IMPACT_THRESHOLD_MEDIUM)) {
    ...
    warnings.push({
      type: highImpact
        ? WarningLabel.PriceImpactHigh
        : WarningLabel.PriceImpactMedium,
...
export function isPriceImpactWarning(warning: Warning): boolean {
  return (
    warning.type === WarningLabel.PriceImpactMedium ||
    warning.type === WarningLabel.PriceImpactHigh
  );
}
