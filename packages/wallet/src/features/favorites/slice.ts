...
import { Ether } from "darshitswap-sdk-core";
...
const ETH_CURRENCY_ID = idFromCurrency(
...
export const DEFAULT_WATCHED_ADDRESSES = [
  VITALIK_ETH_ADDRESS,
  HAYDEN_ETH_ADDRESS,
];
...
export const slice = createSlice({
  ...
  reducers: {
    ...
    toggleTokenVisibility: (
      state,
      {
        payload: { currencyId, isSpam },
      }: PayloadAction<{ currencyId: string; isSpam?: boolean }>
    ) => {
      const isVisible =
        state.tokensVisibility[currencyId]?.isVisible ?? isSpam === false;
      state.tokensVisibility[currencyId] = { isVisible: !isVisible };
    },
    toggleNftVisibility: (
      state,
      {
        payload: { nftKey, isSpam },
      }: PayloadAction<{ nftKey: string; isSpam?: boolean }>
    ) => {
      const isVisible =
        state.nftsVisibility[nftKey]?.isVisible ?? isSpam === false;
