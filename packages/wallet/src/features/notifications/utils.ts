...
export const formWCNotificationTitle = (
  appNotification: WalletConnectNotification
): string => {
...
export const formTransferNFTNotificationTitle = (
  ...
  const nftName =
    nft?.name ?? `NFT ${shortenAddress(tokenAddress)} #${tokenId}`;
  const shortenedAddressOrENS = getShortenedAddressOrEns(senderOrRecipient);
...
export const formUnknownTxTitle = (
  ...
  if (txStatus === TransactionStatus.Success) {
    if (target) {
      return i18n.t("notification.transaction.unknown.success.full", {
        addressOrEnsName: target,
      });
    }
  ...
  if (target) {
    return i18n.t("notification.transaction.unknown.fail.full", {
      addressOrEnsName: target,
    });
  }
...
const getShortenedAddressOrEns = (addressOrENS: string): string => {
  return getValidAddress(addressOrENS)
