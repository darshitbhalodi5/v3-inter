...
describe(pushTransactionNotification, () => {
      ...
      .withState({
        transactions: {
          [from]: {
            [chainId]: {
              uuid1: {
                typeInfo: TransactionType.Approve,
                addedTime: Date.now(),
              },
              uuid2: {
                typeInfo: TransactionType.Swap,
                addedTime: Date.now() + 3000,
              },
            },
          },
        },
  ...
  it("Handles sending currency", () => {
    ...
    const finalizedSendCurrencyAction =
      createFinalizedTxAction(sendCurrencyTypeInfo);
  ...
  it("Handles receiving currency", () => {
    ...
    const finalizedReceiveCurrencyAction = createFinalizedTxAction(
      receiveCurrencyTypeInfo
    );
    const { chainId, from, hash } = finalizedReceiveCurrencyAction.payload;

    return expectSaga(
      pushTransactionNotification,
      finalizedReceiveCurrencyAction
    )
      .put(
        pushNotification({
          txStatus: TransactionStatus.Success,
  ...
  it("Handles receiving NFTs", () => {
    ...
    const finalizedReceiveNftAction =
      createFinalizedTxAction(receiveNftTypeInfo);
