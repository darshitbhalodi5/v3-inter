...
export const getOnChainBalancesFetch = async (
  ...
  if (!currencyAddress || !chainId || !accountAddress) {
    throw new Error(
      `currencyAddress, chainId, or accountAddress is not defined`
    );
  }
...
export function useOnChainNativeCurrencyBalance(
  ...
  const { balance, isLoading } = useOnChainCurrencyBalance(
    currency,
    accountAddress
  );
  return {
    balance: balance as CurrencyAmount<NativeCurrencyClass> | undefined,
    isLoading,
  };
}
