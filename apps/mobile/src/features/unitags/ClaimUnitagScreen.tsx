...
import { ADDRESS_ZERO } from "darshitswap-v3-sdk";
import {
  AnimatePresence,
  AnimatedFlex,
  Button,
...
} from "wallet/src/features/onboarding/types";
import {
  UNITAG_SUFFIX,
...
const UNITAG_NAME_ANIMATE_DISTANCE_Y =
  imageSizes.image100 + spacing.spacing48 + spacing.spacing24;
...
export function ClaimUnitagScreen({ navigation, route }: Props): JSX.Element {
  ...
  const inputPlaceholder = getYourNameString(
    t("unitags.claim.username.default")
  );
  ...
  const pendingAccountAddress = Object.values(usePendingAccounts())?.[0]
    ?.address;
  const unitagAddress = address || pendingAccountAddress;

  const [showInfoModal, setShowInfoModal] = useState(false);
  const [showClaimPeriodInfoModal, setShowClaimPeriodInfoModal] =
    useState(false);

  const [showTextInputView, setShowTextInputView] = useState(true);
  const [unitagInputValue, setUnitagInputValue] = useState<string | undefined>(
    undefined
  );
  const [isCheckingUnitag, setIsCheckingUnitag] = useState(false);
  const [shouldBlockContinue, setShouldBlockContinue] = useState(false);
  const [unitagToCheck, setUnitagToCheck] = useState<string | undefined>(
    undefined
  );
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener("focus", () => {
      ...
      setTimeout(() => {
        ...
        addressViewOpacity.value = withTiming(1, {
          duration: ONE_SECOND_MS / 2,
        });
      }, ONE_SECOND_MS);
    });
  ...
  const onPressMaybeLater = (): void => {
    sendWalletAnalyticsEvent(UnitagEventName.UnitagOnboardingActionTaken, {
      action: "later",
    });
    ...
    >
      ...
      >
        ...
        <AnimatedFlex
          ...
          style={{
            transform: [{ translateY: unitagInputContainerTranslateY }],
          }}
        >
          {!showTextInputView && (
      ...
      <Flex gap="$spacing24" justifyContent="flex-end">
        ...
        <Button
          disabled={
            !unitagInputValue || isCheckingUnitag || shouldBlockContinue
          }
          size="medium"
...
const InfoModal = ({
  ...
  const colors = useSporeColors();
  const { t } = useTranslation();
  const usernamePlaceholder = getYourNameString(
    t("unitags.claim.username.default")
  );
