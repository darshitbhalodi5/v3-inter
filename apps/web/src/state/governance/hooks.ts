...
export function useAllProposalData(): {
  ...
  const gov0ProposalIndexes = useMemo(() => {
    return chainId === ChainId.MAINNET
      ? V0_PROPOSAL_IDS
      : countToIndices(proposalCount0);
  }, [chainId, proposalCount0]);
  const gov1ProposalIndexes = useMemo(() => {
    return chainId === ChainId.MAINNET
      ? V1_PROPOSAL_IDS
      : countToIndices(proposalCount1);
  ...
  const proposalsV2 = useSingleContractMultipleData(
    gov2,
    "proposals",
    gov2ProposalIndexes,
  );
  ...
  const proposalStatesV0 = useSingleContractMultipleData(
    gov0,
    "state",
    gov0ProposalIndexes,
  );
  const proposalStatesV1 = useSingleContractMultipleData(
    gov1,
    "state",
    gov1ProposalIndexes,
  );
  const proposalStatesV2 = useSingleContractMultipleData(
    gov2,
    "state",
    gov2ProposalIndexes,
  );
  ...
  const formattedLogsV0 = useFormattedProposalCreatedLogs(
    gov0,
    gov0ProposalIndexes,
    11042287,
    12563484,
    ...
    return {
      data: proposalsCallData.map((proposal, i) => {
        ...
        return {
          ...
          forCount: CurrencyAmount.fromRawAmount(
            uni,
            proposal?.result?.forVotes,
          ),
          againstCount: CurrencyAmount.fromRawAmount(
            uni,
          ...
          governorIndex:
            i >= proposalsV0.length + proposalsV1.length
              ? 2
              : i >= proposalsV0.length
                ? 1
                : 0,
        };
      }),
...
export function useUserVotes(): {
  ...
  return useMemo(() => {
    ...
    return {
      ...
      votes:
        uni && result
          ? CurrencyAmount.fromRawAmount(uni, result?.[0])
          : undefined,
    };
