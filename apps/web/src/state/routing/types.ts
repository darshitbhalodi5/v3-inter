...
import { MixedRouteSDK, ONE, Protocol, Trade } from "darshitswap-router-sdk";
import {
  ChainId,
  Currency,
  CurrencyAmount,
...
import { Route as V2Route } from "darshitswap-v2-sdk";
import { Route as V3Route } from "darshitswap-v3-sdk";
...
export class ClassicTrade extends Trade<Currency, Currency, TradeType> {
  ...
  public get postSwapFeeOutputAmount(): CurrencyAmount<Currency> {
    ...
    const swapFeeAmount = CurrencyAmount.fromRawAmount(
      this.outputAmount.currency,
      this.swapFee?.amount ?? 0,
    );
...
export const OFFCHAIN_ORDER_TYPE_TO_SIGNATURE_TYPE: Record<
  OffchainOrderType,
  SignatureType
> = {
...
export class LimitOrderTrade {
  ...
  public asDutchOrderTrade(options?: {
    ...
    const swapperOutput = {
      token: this.amountOut.currency.isNative
        ? AddressZero
        : this.amountOut.currency.address,
    ...
    const swapFee = this.swapFee && {
      ...
      startAmount: BigNumber.from(
        this.amountOut.multiply(this.swapFee.percent).quotient.toString(),
      ),
    ...
    return new IDutchOrderTrade({
      ...
      orderInfo: {
        ...
        input: {
          token: this.amountIn.currency.isNative
            ? AddressZero
            : this.amountIn.currency.address,
