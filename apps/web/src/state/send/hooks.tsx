...
export function useDerivedSendInfo(state: SendState): SendInfo {
  ...
  const exactAmountOut = useUSDTokenUpdater(
    inputInFiat,
  ...
  const parsedTokenAmount = useMemo(() => {
    return tryParseCurrencyAmount(
      inputInFiat ? exactAmountOut : exactAmountToken,
      inputCurrency,
    );
  ...
  const inputError = useMemo(() => {
    const insufficientBalance =
      parsedTokenAmount && inputCurrencyBalance?.lessThan(parsedTokenAmount);
    ...
    let totalAmount = CurrencyAmount.fromRawAmount(
      nativeCurrency,
      gasFee?.value,
    );
    if (
      parsedTokenAmount &&
      inputCurrency &&
      nativeCurrency?.equals(inputCurrency)
    ) {
