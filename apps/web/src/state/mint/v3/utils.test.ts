...
describe("hooks", () => {
  describe("#tryParsePrice", () => {
    ...
    it("should return a price when decimals are the same", () => {
      ...
      expect(
        tryParsePrice(baseToken, quoteToken, "20")?.toSignificant(6),
      ).toEqual("20");
      expect(
        tryParsePrice(baseToken, quoteToken, "20.05")?.toSignificant(6),
      ).toEqual("20.05");
      expect(
        tryParsePrice(baseToken, quoteToken, "20.123456789")?.toSignificant(6),
      ).toEqual("20.1235");
      expect(
        tryParsePrice(baseToken, quoteToken, "0.123456789")?.toSignificant(6),
      ).toEqual("0.123457");
      expect(
        tryParsePrice(baseToken, quoteToken, ".123456789")?.toSignificant(6),
      ).toEqual("0.123457");
      expect(
        tryParsePrice(
          baseToken,
      ...
      expect(
        tryParsePrice(
          baseToken,
          quoteToken,
          /* ~2^-128 */ "0.000000000000000000000000000587747",
        )?.toSignificant(6),
