import { Currency, CurrencyAmount, Price, TradeType } from "darshitswap-sdk-core";
...
export function getDefaultPriceInverted(
  inputCurrency?: Currency,
  outputCurrency?: Currency,
): boolean {
  const [isInputStablecoin, isOutputStablecoin] = [
    isStablecoin(inputCurrency),
    isStablecoin(outputCurrency),
  ];
...
function useLimitOrderTrade({
  ...
  useEffect(() => {
    async function calculateWrapInfo() {
      ...
      const [currencyIn, needsWrap] = inputCurrency.isNative
        ? [inputCurrency.wrapped, true]
        : [inputCurrency, false];
...
function isNativeOrWrappedNative(currency: Currency) {
  return (
    currency.isNative ||
    nativeOnChain(currency.chainId).wrapped.equals(currency)
  );
}
...
function useMarketPriceAndFee(
  ...
  const baseCurrencyAmount =
    ...
    CurrencyAmount.fromRawAmount(
      nativeOnChain(inputCurrency.chainId),
      10 ** 18,
    );
  ...
  const marketPrice: Price<Currency, Currency> | undefined = useMemo(() => {
    ...
    if (isNativeOrWrappedNative(inputCurrency)) {
      if (
        !tradeB?.outputAmount.currency.equals(outputCurrency) ||
        !isClassicTrade(tradeB)
      )
        return undefined;
      ...
      return new Price(
        inputCurrency,
        outputCurrency,
        referencePrice.denominator,
        referencePrice.numerator,
      );
    }
    ...
    if (isNativeOrWrappedNative(outputCurrency)) {
      if (
        !tradeA?.inputAmount.currency.equals(inputCurrency) ||
        !isClassicTrade(tradeA)
      )
        return undefined;
  ...
  const fee = useMemo(() => {
    ...
    const canTakeFees =
      tradeA.swapFee?.percent.greaterThan(0) &&
      tradeB.swapFee?.percent.greaterThan(0);
