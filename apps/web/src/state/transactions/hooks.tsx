...
import { ChainId, SUPPORTED_CHAINS, Token } from "darshitswap-sdk-core";
...
export function useTransactionAdder(): (
  ...
  return useCallback(
    (
      ...
      dispatch(
        addTransaction({ hash, from: account, info, chainId, nonce, deadline }),
      );
    },
...
function usePendingApprovalAmount(
  token?: Token,
  spender?: string,
): BigNumber | undefined {
  const allTransactions = useAllTransactions();
  return useMemo(() => {
    ...
    for (const txHash in allTransactions) {
      ...
      if (!tx || tx.receipt || tx.info.type !== TransactionType.APPROVAL)
        continue;
      if (
...
export function useHasPendingRevocation(
  token?: Token,
  spender?: string,
): boolean {
  ...
  return useMemo(
    () =>
      Object.values(allTransactions).filter(
        (tx) => tx.from === account && isPendingTx(tx),
      ),
