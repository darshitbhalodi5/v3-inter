...
import { FeeAmount, TICK_SPACINGS } from "darshitswap-v3-sdk";
...
describe("#computeSurroundingTicks", () => {
  it("correctly compute active liquidity", () => {
    ...
    expect(
      previous.map((t) => [t.tick, parseFloat(t.liquidityActive.toString())]),
    ).toEqual([
    ...
    expect(
      subsequent.map((t) => [t.tick, parseFloat(t.liquidityActive.toString())]),
    ).toEqual([
