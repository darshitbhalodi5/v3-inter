...
describe("computeUniCirculation", () => {
  ...
  it("before staking", () => {
    expect(computeUniCirculation(token, BigNumber.from(0), undefined)).toEqual(
      tokenAmount(150_000_000),
    );
    expect(
      computeUniCirculation(token, BigNumber.from(1600387200), undefined),
    ).toEqual(tokenAmount(150_000_000));
  });
  it("mid staking", () => {
    expect(
      computeUniCirculation(
        token,
        BigNumber.from(1600387200 + 15 * 24 * 60 * 60),
        undefined,
      ),
    ).toEqual(tokenAmount(155_000_000));
  });
  it("after staking and treasury vesting cliff", () => {
    expect(
      computeUniCirculation(
        token,
        BigNumber.from(1600387200 + 60 * 24 * 60 * 60),
        undefined,
      ),
    ).toEqual(tokenAmount(224_575_341));
  });
  it("subtracts unclaimed uni", () => {
    ...
    ).toEqual(tokenAmount(154_999_000));
  });
});
