...
import { CurrencyAmount, Percent } from "darshitswap-sdk-core";
...
describe("formatNumber", () => {
  beforeEach(() => {
    ...
    mocked(useFeatureFlag).mockImplementation(
      (f) => f === FeatureFlags.CurrencyConversion,
    );
  });
  ...
  it("formats token reference numbers correctly", () => {
    ...
    expect(
      formatNumber({ input: 1234567000000000, type: NumberType.TokenNonTx }),
    ).toBe(">999T");
    expect(formatNumber({ input: 1002345, type: NumberType.TokenNonTx })).toBe(
      "1.00M",
    );
    expect(formatNumber({ input: 1234, type: NumberType.TokenNonTx })).toBe(
      "1,234.00",
    );
    expect(formatNumber({ input: 0.00909, type: NumberType.TokenNonTx })).toBe(
      "0.009",
    );
    expect(formatNumber({ input: 0.09001, type: NumberType.TokenNonTx })).toBe(
      "0.090",
    );
    expect(formatNumber({ input: 0.00099, type: NumberType.TokenNonTx })).toBe(
      "<0.001",
    );
  ...
  it("formats token transaction numbers correctly", () => {
    ...
    expect(
      formatNumber({ input: 1234567.8901, type: NumberType.TokenTx }),
    ).toBe("1,234,567.89");
    expect(formatNumber({ input: 765432.1, type: NumberType.TokenTx })).toBe(
      "765,432.10",
    );

    expect(formatNumber({ input: 7654.321, type: NumberType.TokenTx })).toBe(
      "7,654.32",
    );
    expect(formatNumber({ input: 765.4321, type: NumberType.TokenTx })).toBe(
      "765.432",
    );
    expect(formatNumber({ input: 76.54321, type: NumberType.TokenTx })).toBe(
      "76.5432",
    );
    expect(formatNumber({ input: 7.654321, type: NumberType.TokenTx })).toBe(
      "7.65432",
    );
    expect(
      formatNumber({ input: 7.60000054321, type: NumberType.TokenTx }),
    ).toBe("7.60");
    ...
    expect(formatNumber({ input: 0.987654321, type: NumberType.TokenTx })).toBe(
      "0.98765",
    );
    expect(formatNumber({ input: 0.9, type: NumberType.TokenTx })).toBe("0.90");
    expect(formatNumber({ input: 0.901000123, type: NumberType.TokenTx })).toBe(
      "0.901",
    );
    expect(formatNumber({ input: 0.000000001, type: NumberType.TokenTx })).toBe(
      "<0.00001",
    );
  ...
  it("formats fiat estimates on token details pages correctly", () => {
    ...
    expect(
      formatNumber({ input: 1234567.891, type: NumberType.FiatTokenDetails }),
    ).toBe("$1.23M");
    expect(
      formatNumber({ input: 1234.5678, type: NumberType.FiatTokenDetails }),
    ).toBe("$1,234.57");
    expect(
      formatNumber({ input: 1.048942, type: NumberType.FiatTokenDetails }),
    ).toBe("$1.049");

    expect(
      formatNumber({ input: 0.001231, type: NumberType.FiatTokenDetails }),
    ).toBe("$0.00123");
    expect(
      formatNumber({ input: 0.00001231, type: NumberType.FiatTokenDetails }),
    ).toBe("$0.0000123");

    expect(
      formatNumber({ input: 0.0000001234, type: NumberType.FiatTokenDetails }),
    ).toBe("$0.000000123");
    expect(
      formatNumber({
  ...
  it("formats fiat estimates for tokens correctly", () => {
    ...
    expect(
      formatNumber({ input: 1234567.891, type: NumberType.FiatTokenPrice }),
    ).toBe("$1.23M");
    expect(
      formatNumber({ input: 1234.5678, type: NumberType.FiatTokenPrice }),
    ).toBe("$1,234.57");

    expect(
      formatNumber({ input: 0.010235, type: NumberType.FiatTokenPrice }),
    ).toBe("$0.0102");
    expect(
      formatNumber({ input: 0.001231, type: NumberType.FiatTokenPrice }),
    ).toBe("$0.00123");
    expect(
      formatNumber({ input: 0.00001231, type: NumberType.FiatTokenPrice }),
    ).toBe("$0.0000123");

    expect(
      formatNumber({ input: 0.0000001234, type: NumberType.FiatTokenPrice }),
    ).toBe("$0.000000123");
    expect(
      formatNumber({ input: 0.000000009876, type: NumberType.FiatTokenPrice }),
    ).toBe("<$0.00000001");
    expect(
      formatNumber({
  ...
  it("formats fiat estimates for tokens correctly with spanish locale and yen currency", () => {
    ...
    expect(
      formatNumber({ input: 12345.678, type: NumberType.FiatTokenPrice }),
    ).toBe("12.345,68\xa0¥");
  ...
  it("formats fiat estimates for token stats correctly", () => {
    ...
    expect(
      formatNumber({ input: 1234576, type: NumberType.FiatTokenStats }),
    ).toBe("$1.2M");
    expect(
      formatNumber({ input: 234567, type: NumberType.FiatTokenStats }),
    ).toBe("$234.6K");
    expect(
      formatNumber({ input: 123.456, type: NumberType.FiatTokenStats }),
    ).toBe("$123.46");
    expect(formatNumber({ input: 1.23, type: NumberType.FiatTokenStats })).toBe(
      "$1.23",
    );
    expect(
      formatNumber({ input: 0.123, type: NumberType.FiatTokenStats }),
    ).toBe("$0.12");
    expect(
      formatNumber({ input: 0.00123, type: NumberType.FiatTokenStats }),
    ).toBe("<$0.01");
  ...
  it("formats gas USD prices correctly", () => {
    ...
    expect(
      formatNumber({ input: 1234567.891, type: NumberType.FiatGasPrice }),
    ).toBe("$1.23M");
    expect(formatNumber({ input: 18.448, type: NumberType.FiatGasPrice })).toBe(
      "$18.45",
    );
    expect(formatNumber({ input: 0.0099, type: NumberType.FiatGasPrice })).toBe(
      "<$0.01",
    );
    expect(formatNumber({ input: 0, type: NumberType.FiatGasPrice })).toBe(
      "$0",
    );
  });
  ...
  it("formats USD token quantities prices correctly", () => {
    ...
    expect(
      formatNumber({ input: 1234567.891, type: NumberType.FiatTokenQuantity }),
    ).toBe("$1.23M");
    expect(
      formatNumber({ input: 18.448, type: NumberType.FiatTokenQuantity }),
    ).toBe("$18.45");
    expect(
      formatNumber({ input: 0.0099, type: NumberType.FiatTokenQuantity }),
    ).toBe("<$0.01");
    expect(formatNumber({ input: 0, type: NumberType.FiatTokenQuantity })).toBe(
      "$0.00",
    );
  });
  ...
  it("formats Swap text input/output numbers correctly", () => {
    ...
    expect(
      formatNumber({ input: 1234567.8901, type: NumberType.SwapTradeAmount }),
    ).toBe("1234570");
    expect(
      formatNumber({ input: 765432.1, type: NumberType.SwapTradeAmount }),
    ).toBe("765432");

    expect(
      formatNumber({ input: 7654.321, type: NumberType.SwapTradeAmount }),
    ).toBe("7654.32");
    expect(
      formatNumber({ input: 765.4321, type: NumberType.SwapTradeAmount }),
    ).toBe("765.432");
    expect(
      formatNumber({ input: 76.54321, type: NumberType.SwapTradeAmount }),
    ).toBe("76.5432");
    expect(
      formatNumber({ input: 7.654321, type: NumberType.SwapTradeAmount }),
    ).toBe("7.65432");
    expect(
      formatNumber({ input: 7.60000054321, type: NumberType.SwapTradeAmount }),
    ).toBe("7.60");
    expect(formatNumber({ input: 7.6, type: NumberType.SwapTradeAmount })).toBe(
      "7.60",
    );
    expect(formatNumber({ input: 7, type: NumberType.SwapTradeAmount })).toBe(
      "7.00",
    );

    expect(
      formatNumber({ input: 0.987654321, type: NumberType.SwapTradeAmount }),
    ).toBe("0.98765");
    expect(formatNumber({ input: 0.9, type: NumberType.SwapTradeAmount })).toBe(
      "0.90",
    );
    expect(
      formatNumber({ input: 0.901000123, type: NumberType.SwapTradeAmount }),
    ).toBe("0.901");
    expect(
      formatNumber({ input: 0.000000001, type: NumberType.SwapTradeAmount }),
    ).toBe("0.000000001");
    expect(formatNumber({ input: 0, type: NumberType.SwapTradeAmount })).toBe(
      "0",
    );
  });
  ...
  it("formats NFT numbers correctly", () => {
    ...
    expect(
      formatNumber({ input: 1002345, type: NumberType.NFTTokenFloorPrice }),
    ).toBe("1M");
    expect(
      formatNumber({ input: 1234, type: NumberType.NFTTokenFloorPrice }),
    ).toBe("1.23K");
    expect(
      formatNumber({ input: 12.34467, type: NumberType.NFTTokenFloorPrice }),
    ).toBe("12.34");
    expect(
      formatNumber({ input: 12.1, type: NumberType.NFTTokenFloorPrice }),
    ).toBe("12.1");
    expect(
      formatNumber({ input: 0.00909, type: NumberType.NFTTokenFloorPrice }),
    ).toBe("0.009");
    expect(
      formatNumber({ input: 0.09001, type: NumberType.NFTTokenFloorPrice }),
    ).toBe("0.09");
    expect(
      formatNumber({ input: 0.00099, type: NumberType.NFTTokenFloorPrice }),
    ).toBe("<0.001");
    expect(
      formatNumber({ input: 0, type: NumberType.NFTTokenFloorPrice }),
    ).toBe("0");
    ...
    expect(
      formatNumber({ input: 0.987654321, type: NumberType.NFTCollectionStats }),
    ).toBe("1");
    expect(
      formatNumber({ input: 0.9, type: NumberType.NFTCollectionStats }),
    ).toBe("1");
    expect(
      formatNumber({ input: 76543.21, type: NumberType.NFTCollectionStats }),
    ).toBe("76.5K");
    expect(
      formatNumber({
        input: 7.60000054321,
    ...
    expect(
      formatNumber({ input: 1234567890, type: NumberType.NFTCollectionStats }),
    ).toBe("1.2B");
    expect(
      formatNumber({
        input: 1234567000000000,
...
describe("formatUSDPrice", () => {
  ...
  it("format fiat price correctly in euros with french locale", () => {
    ...
    expect(formatFiatPrice({ price: 1_000_000_000_000 })).toBe(
      "1,00\xa0Bn\xa0€",
    );
    expect(formatFiatPrice({ price: 1_000_000_000_000_000 })).toBe(
      "1000,00\xa0Bn\xa0€",
    );
  });
});
...
describe("formatReviewSwapCurrencyAmount", () => {
  ...
  it("should use TokenTx formatting under a default length", () => {
    const { formatReviewSwapCurrencyAmount } = renderHook(() => useFormatter())
      .result.current;
...
describe("formatToFiatAmount", () => {
  ...
  it("should return default values when undefined", () => {
    ...
    const { convertToFiatAmount } = renderHook(() => useFormatter()).result
      .current;
