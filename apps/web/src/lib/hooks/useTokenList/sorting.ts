...
function tokenComparator(balances: TokenBalances, a: Token, b: Token) {
  ...
  const balanceComparison = balanceComparator(
    balances[aAddress]?.usdValue,
    balances[bAddress]?.usdValue,
  );
...
export function getSortedPortfolioTokens(
  portfolioTokenBalances:
    | readonly (PortfolioTokenBalancePartsFragment | undefined)[]
    | undefined,
  balances: TokenBalances,
  chainId: ChainId | undefined,
  splitOptions?: SplitOptions,
): Token[] {
  const validVisiblePortfolioTokens = splitHiddenTokens(
    portfolioTokenBalances ?? [],
    splitOptions,
  )
