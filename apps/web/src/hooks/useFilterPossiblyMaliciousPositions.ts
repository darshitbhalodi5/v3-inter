...
export function useFilterPossiblyMaliciousPositions(
  ...
  const nonListPositionTokenAddresses = useMemo(
    () =>
      getUniqueAddressesFromPositions(positions).filter(
        (address) => !activeTokensList[address],
      ),
  ...
  return useMemo(
    () =>
      positions.filter((position) => {
        ...
        const token0FromList = activeTokensList[position.token0] as
          | Token
          | undefined;
        const token1FromList = activeTokensList[position.token1] as
          | Token
          | undefined;
        ...
        if (
          hasURL(token0FromList?.symbol ?? addressesToSymbol[position.token0])
        )
          urlSymbolCount++;
        if (
          hasURL(token1FromList?.symbol ?? addressesToSymbol[position.token1])
        )
          urlSymbolCount++;
