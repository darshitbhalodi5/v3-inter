import { Currency, Token } from "darshitswap-sdk-core";
...
export function useFeeTierDistribution(
  ...
  return useMemo(() => {
    ...
    const largestUsageFeeTier = Object.keys(distributions)
      ...
      .filter(
        (d: FeeAmount) =>
          distributions[d] !== 0 && distributions[d] !== undefined,
      )
      .reduce(
        (a: FeeAmount, b: FeeAmount) =>
          (distributions[a] ?? 0) > (distributions[b] ?? 0) ? a : b,
        -1,
      );
    ...
    return {
      ...
      largestUsageFeeTier:
        largestUsageFeeTier === -1 ? undefined : largestUsageFeeTier,
    };
  }, [
    isLoading,
    error,
...
function usePoolTVL(token0: Token | undefined, token1: Token | undefined) {
  ...
  return useMemo(() => {
    ...
    const tvlByFeeTier = all.reduce<{
      ...
      (acc, value) => {
        acc[value.feeTier][0] =
          (acc[value.feeTier][0] ?? 0) + Number(value.totalValueLockedToken0);
        acc[value.feeTier][1] =
          (acc[value.feeTier][1] ?? 0) + Number(value.totalValueLockedToken1);
        return acc;
      },
      {
