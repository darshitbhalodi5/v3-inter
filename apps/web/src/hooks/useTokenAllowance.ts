...
import { CurrencyAmount, MaxUint256, Token } from "darshitswap-sdk-core";
      ...
      trace({ name: "Allowance", op: "permit.allowance" }, async (trace) => {
        try {
          ...
          const response = await trace.child(
            { name: "Approve", op: "wallet.approve" },
            async (walletTrace) => {
              try {
          ...
          if (error instanceof UserRejectedRequestError) {
            ...
            const symbol = amount?.currency.symbol ?? "Token";
            throw new Error(
              `${symbol} token allowance failed: ${error instanceof Error ? error.message : error}`,
            );
          }
        }
      }),
