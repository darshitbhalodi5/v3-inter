...
export function useUSDTokenUpdater(
  ...
  return useMemo(() => {
    ...
    if (isFiatInput) {
      const exactAmountUSD = (
        parseFloat(exactAmount || "0") / conversionRate
      ).toFixed(NUM_DECIMALS_USD);
      ...
      const currencyAmount = stablecoinAmount
        ? price?.invert().quote(stablecoinAmount)
        : undefined;
    ...
    const exactCurrencyAmount = tryParseCurrencyAmount(
      exactAmount || "0",
      exactCurrency,
    );

    const usdPrice = exactCurrencyAmount
      ? price?.quote(exactCurrencyAmount)
      : undefined;
    const fiatPrice = convertToFiatAmount(
      parseFloat(usdPrice?.toExact() ?? "0"),
    ).amount;
    const formattedFiatPrice = fiatPrice
      ? fiatPrice.toFixed(NUM_DECIMALS_DISPLAY)
      : "0";
