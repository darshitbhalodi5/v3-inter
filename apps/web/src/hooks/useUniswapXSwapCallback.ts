...
async function getUpdatedNonce(
  ...
  try {
    ...
    const res = await fetch(
      `${UNISWAP_GATEWAY_DNS_URL}/nonce?address=${swapper.toLowerCase()}&chainId=${chainId}`,
    );
    ...
    () =>
      trace({ name: "Swap (Dutch)", op: "swap.x.dutch" }, async (trace) => {
        ...
        try {
          ...
          const startTime =
            Math.floor(Date.now() / 1000) + trade.startTimeBufferSecs;
          ...
          const signature = await trace.child(
            { name: "Sign", op: "wallet.sign" },
            async (walletTrace) => {
              try {
