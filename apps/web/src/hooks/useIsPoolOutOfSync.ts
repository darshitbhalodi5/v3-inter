...
function useMarketPrice(baseCurrency?: Currency, quoteCurrency?: Currency) {
  ...
  const baseCurrencyStableCoinAmount = useStablecoinAmountFromFiatValue(
    baseCurrencyUSDPrice.data,
  );
  ...
  const quoteCurrencyStableCoinAmount = useStablecoinAmountFromFiatValue(
    quoteCurrencyUSDPrice.data,
  );
...
export function useIsPoolOutOfSync(poolPrice?: Price<Token, Token>) {
  ...
  const scaledMarketPrice = JSBI.BigInt(
    marketPrice.multiply(DECIMAL_SCALAR).toFixed(0),
  );
