...
export default function useClassicAutoSlippageTolerance(
  ...
  const gasCostUSD = useUSDPrice(
    nativeCurrency && nativeGasCost
      ? CurrencyAmount.fromRawAmount(nativeCurrency, nativeGasCost)
      : undefined,
  );
  const gasCostStablecoinAmount = useStablecoinAmountFromFiatValue(
    gasCostUSD.data,
  );
  ...
  return useMemo(() => {
    ...
    if (outputDollarValue && dollarCostToUse) {
      ...
      const fraction = dollarCostToUse.asFraction.divide(
        outputDollarValue.asFraction,
      );
