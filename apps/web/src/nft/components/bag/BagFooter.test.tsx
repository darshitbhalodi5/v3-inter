...
import { ChainId, CurrencyAmount, Percent } from "darshitswap-sdk-core";
import { UNIVERSAL_ROUTER_ADDRESS } from "darshitswap-universal-router-sdk";
import { useWeb3React } from "@web3-react/core";
import { nativeOnChain } from "constants/tokens";
import {
  getURAddress,
  useNftUniversalRouterAddress,
} from "graphql/data/nft/NftUniversalRouterAddress";
...
jest.mock("graphql/data/nft/NftUniversalRouterAddress", () => {
  const originalModule = jest.requireActual(
    "graphql/data/nft/NftUniversalRouterAddress",
  );
  return {
    ...originalModule,
...
const getBuyButton = () =>
  screen.queryByTestId("nft-buy-button") as HTMLButtonElement;
const getBuyButtonWarning = () =>
  screen.queryByTestId("nft-buy-button-warning") as HTMLDivElement;
...
describe("BagFooter.tsx", () => {
  ...
  it("transaction error", () => {
    ...
    expect(buyButtonWarning.textContent).toBe(
  ...
  it("invalid token trade", () => {
    ...
    mocked(useTokenBalance).mockReturnValue(
      toCurrencyAmount(TEST_TOKEN_1, 1000),
    );
    mocked(useDerivedPayWithAnyTokenSwapInfo).mockReturnValue({
      state: TradeState.INVALID,
      trade: TEST_TRADE_EXACT_INPUT,
  ...
  it("should use the correct UR address", () => {
    ...
    expect(getURAddress(ChainId.MAINNET)).toBe(
      UNIVERSAL_ROUTER_ADDRESS(ChainId.MAINNET),
    );
    expect(getURAddress(ChainId.MAINNET, "test_nft_ur_address")).toBe(
      "test_nft_ur_address",
    );
    expect(getURAddress(ChainId.OPTIMISM)).toBe(
      UNIVERSAL_ROUTER_ADDRESS(ChainId.OPTIMISM),
    );
