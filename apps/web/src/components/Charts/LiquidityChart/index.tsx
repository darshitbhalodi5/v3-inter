...
export class LiquidityBarChartModel extends ChartModel<LiquidityBarData> {
  ...
  fitContent() {
    ...
    const activeTickIndex = this.data.findIndex(
      (bar) => bar.tick === this.activeTick,
    );
...
async function calculateActiveRangeTokensLocked(
  ...
  try {
    ...
    const token1Amount = (
      await pool1.getOutputAmount(maxAmount(token0), bottomOfRangePrice)
    )[0];
    const amount0Locked = parseFloat(
      tick.sdkPrice.invert().quote(token1Amount).toExact(),
    );
    ...
    const token0Amount = (
      await pool1.getOutputAmount(maxAmount(token1), topOfRangePrice)
    )[0];
    const amount1Locked = parseFloat(
      tick.sdkPrice.quote(token0Amount).toExact(),
    );
...
async function calculateTokensLocked(
  ...
  try {
    ...
    const maxAmountToken0 = CurrencyAmount.fromRawAmount(
      token0,
  ...
  useEffect(() => {
    async function formatData() {
      ...
      for (let index = 0; index < ticksProcessed.length; index++) {
        ...
        if (
          isActive &&
          activePoolData.activeTick &&
          activePoolData.currentTick
        ) {
