...
export function GasBreakdownTooltip({ trade }: GasBreakdownTooltipProps) {
  ...
  const approvalEstimate = trade.approveInfo.needsApprove
    ...
    : undefined;
  const wrapEstimate =
    isUniswapX && trade.wrapInfo.needsWrap
      ? trade.wrapInfo.wrapGasEstimateUSD
      : undefined;
  const showEstimateDetails = Boolean(wrapEstimate || approvalEstimate);

  const description = isUniswapX ? (
    <UniswapXDescription />
  ) : (
    <NetworkCostDescription native={native} />
  );
  ...
  return (
    <Container gap="md">
      <AutoColumn gap="sm">
        <GasCostItem
          title={<Trans>Wrap {{ sym: native.symbol }}</Trans>}
          amount={wrapEstimate}
        />
        <GasCostItem
          title={
            <Trans>Allow {{ sym: inputCurrency.symbol }} (one time)</Trans>
          }
