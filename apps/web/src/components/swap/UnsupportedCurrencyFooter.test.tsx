...
import { Token } from "darshitswap-sdk-core";
...
const unsupportedToken = new Token(
  1,
...
describe("UnsupportedCurrencyFooter.tsx with unsupported tokens", () => {
  ...
  it("renders", () => {
    const { asFragment } = render(
      <UnsupportedCurrencyFooter show={true} currencies={[unsupportedToken]} />,
    );
  ...
  it("works as expected when one unsupported token exists", async () => {
    ...
    expect(
      within(unsupportedCard).getByText(unsupportedTokenSymbol),
    ).toBeInTheDocument();
    expect(
      within(unsupportedCard).getByText(unsupportedTokenAddress).closest("a"),
    ).toHaveAttribute("href", unsupportedTokenExplorerLink);
    await act(() => userEvent.click(screen.getByTestId("close-icon")));
    await waitForElementToBeRemoved(
      rendered.queryByTestId("unsupported-token-card"),
    );
    expect(rendered.queryByText("Unsupported Assets")).toBeNull();
    expect(rendered.queryByTestId("unsupported-token-card")).toBeNull();
    expect(
      rendered.queryByText((content) =>
        content.startsWith(
          "Some assets are not available through this interface",
        ),
      ),
