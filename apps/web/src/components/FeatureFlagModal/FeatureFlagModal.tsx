import { ChainId } from "darshitswap-sdk-core";
import { SmallButtonPrimary } from "components/Button";
import Column from "components/Column";
import {
  QUICK_ROUTE_CONFIG_KEY,
  useQuickRouteChains,
} from "featureFlags/dynamicConfig/quickRouteChains";
...
import {
  DynamicConfigs,
  getConfigName,
} from "uniswap/src/features/experiments/configs";
import {
  FeatureFlags,
  getFeatureFlagName,
...
const FlagsColumn = styled(Column)`
  ...
  scrollbar-color: white rgb(19, 17, 24);
...
function Variant({ option }: { option: string }) {
  return (
    <option value={option} style={{ background: "#FFFFFF", color: "#000000" }}>
      {option}
    </option>
  );
}
    ...
    <Modal open={open}>
      ...
      <FlagsColumn>
        ...
        <FeatureFlagGroup name="Extension">
          <FeatureFlagOption
            flag={FeatureFlags.ExtensionBetaLaunch}
            label="Beta phase of go-to-market campaign"
          />
          <FeatureFlagOption
        ...
        <FeatureFlagGroup name="Outage Banners">
          <FeatureFlagOption
            flag={FeatureFlags.OutageBannerArbitrum}
            label="Outage Banner for Arbitrum"
          />
          <FeatureFlagOption
            flag={FeatureFlags.OutageBannerOptimism}
            label="Outage Banner for Optimism"
          />
          <FeatureFlagOption
            flag={FeatureFlags.OutageBannerPolygon}
            label="Outage Banner for Polygon"
          />
        </FeatureFlagGroup>
        <FeatureFlagGroup name="Debug">
          <FeatureFlagOption
            flag={FeatureFlags.TraceJsonRpc}
