...
export function useCurrencySearchResults({
  ...
  const gqlPopularTokensEmpty =
    (!popularTokens?.topTokens || popularTokens.topTokens.length === 0) &&
      ...
      if (
        ...
        return [
          ...((searchResults?.searchTokens
            ?.map(gqlCurrencyMapper)
            .filter(Boolean) as Currency[]) ?? []),
          ...userAddedTokens
            .filter(getTokenFilter(searchQuery))
            .filter(
              (userAddedToken) =>
                !searchResults?.searchTokens?.find((token) =>
                  isSameAddress(token?.address, userAddedToken.address),
                ),
            ),
        ];
      } else {
    ...
    const currencyFilter = (currency: Currency) => {
      ...
      if (isEmpty(searchQuery) && currency instanceof UserAddedToken) {
        if (
          ...
          otherSelectedCurrency?.equals(currency)
        )
          return true;
        return balanceMap[currency.address.toLowerCase()]?.usdValue > 0;
      }
  ...
  const filteredInactiveTokens = useSearchInactiveTokenLists(
    !filters?.onlyShowCurrenciesWithBalance && sortedCombinedTokens.length === 0
      ? searchQuery
      : undefined,
  );
    ...
    if (!isEmpty(searchQuery) || portfolioTokens.length === 0) {
      return [
        new CurrencyListSectionTitle(
          searchQuery ? t`Search results` : t`Popular tokens`,
        ),
        ...sortedCombinedTokens.map(
          searchQuery ? searchResultsCurrencyListMapper : currencyListRowMapper,
        ),
        ...inactiveTokens.map(currencyListRowMapper),
      ];
    } else if (
      sortedTokensWithoutPortfolio.length === 0 &&
