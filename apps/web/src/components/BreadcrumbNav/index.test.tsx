...
describe("BreadcrumbNav", () => {
  it("renders hover components correctly", async () => {
    ...
    await act(() =>
      userEvent.unhover(screen.getByTestId("current-breadcrumb")),
    );
    expect(
      screen.queryByTestId("breadcrumb-hover-copy"),
  ...
  it("does not display address hover for native tokens", async () => {
    ...
    const { asFragment } = render(
      <CurrentPageBreadcrumb address={NATIVE_CHAIN_ID} currency={ETH} />,
    );
    expect(asFragment()).toMatchSnapshot();
