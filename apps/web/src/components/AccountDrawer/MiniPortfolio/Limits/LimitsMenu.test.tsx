...
jest.mock("components/AccountDrawer/MiniPortfolio/formatTimestamp", () => ({
  ...jest.requireActual(
    "components/AccountDrawer/MiniPortfolio/formatTimestamp",
  ),
...
jest.mock("hooks/Tokens", () => {
  return {
    useCurrency: (address?: string) => {
      ...
      if (
        address?.toLowerCase() === WETH9[ChainId.MAINNET].address.toLowerCase()
      ) {
...
describe("LimitsMenu", () => {
  it("should render when there is one open order", async () => {
    ...
    expect(screen.getByTestId("LimitsMenuContainer").children.length).toEqual(
      1,
    ); // one order
  });
