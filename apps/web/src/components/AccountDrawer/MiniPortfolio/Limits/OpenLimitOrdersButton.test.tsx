...
describe("OpenLimitOrdersButton", () => {
  it("should not render if there are no open limit orders", () => {
    ...
    const { container } = render(
      <OpenLimitOrdersButton account="0x123" openLimitsMenu={jest.fn()} />,
    );
  ...
  it("should call the callback when clicked", () => {
    ...
    const { container } = render(
      <OpenLimitOrdersButton account="0x123" openLimitsMenu={clickCallback} />,
    );
    act(() => {
      fireEvent.click(
        container.firstChild?.firstChild?.firstChild as HTMLElement,
      );
    });
  ...
  it("should have a warning when the limit is almost reached", () => {
    ...
    expect(
      screen.getByText("Approaching 100 limit maximum"),
    ).toBeInTheDocument();
  });
});
