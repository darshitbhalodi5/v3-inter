import { ChainId, Currency, CurrencyAmount, TradeType } from "darshitswap-sdk-core";
...
import { PERMIT2_ADDRESS } from "darshitswap-permit2-sdk";
...
export function useOrderAmounts(order?: UniswapXOrderDetails):
  ...
  const inputCurrency = useCurrency(
    order?.swapInfo?.inputCurrencyId || "ETH",
    order?.chainId,
  );
  const outputCurrency = useCurrency(
    order?.swapInfo?.outputCurrencyId || "ETH",
    order?.chainId,
  );
  ...
  if (swapInfo.tradeType === TradeType.EXACT_INPUT) {
    return {
      inputAmount: CurrencyAmount.fromRawAmount(
        inputCurrency,
        swapInfo.inputCurrencyAmountRaw,
      ),
      outputAmount: CurrencyAmount.fromRawAmount(
        outputCurrency,
        swapInfo.settledOutputCurrencyAmountRaw ??
          swapInfo.expectedOutputCurrencyAmountRaw,
      ),
    };
  } else {
    return {
      inputAmount: CurrencyAmount.fromRawAmount(
        inputCurrency,
        swapInfo.expectedInputCurrencyAmountRaw,
      ),
      outputAmount: CurrencyAmount.fromRawAmount(
        outputCurrency,
        swapInfo.outputCurrencyAmountRaw,
      ),
    };
  }
}
...
function getOrderTitle(order: UniswapXOrderDetails): ReactNode {
  ...
  switch (order.status) {
    case UniswapXOrderStatus.OPEN:
      return isLimit ? (
        <Trans>Limit pending</Trans>
      ) : (
        <Trans>Order pending</Trans>
      );
    case UniswapXOrderStatus.EXPIRED:
      return isLimit ? (
        <Trans>Limit expired</Trans>
      ) : (
    ...
    case UniswapXOrderStatus.CANCELLED:
      return isLimit ? (
        <Trans>Limit cancelled</Trans>
      ) : (
        <Trans>Order cancelled</Trans>
      );
    case UniswapXOrderStatus.FILLED:
      return isLimit ? (
        <Trans>Limit executed</Trans>
      ) : (
        <Trans>Order executed</Trans>
      );
    default:
      return null;
  }
...
export function OrderContent({
  ...
  return (
    <Column>
      ...
      {Boolean(
        order.status === UniswapXOrderStatus.OPEN && order.encodedOrder,
      ) && (
        ...
        <InsufficientFundsCopyContainer>
          ...
          <Column>
            ...
            <ThemedText.SubHeaderSmall lineHeight="20px">
              <Trans>
                This order was canceled because your balance went below the
                input amount.
              </Trans>
  ...
  return (
    <>
      ...
      <Modal
        maxWidth={375}
        isOpen={
          !!selectedOrderAtomValue?.modalOpen &&
          cancelState === CancellationState.NOT_STARTED
        }
        onDismiss={reset}
      >
