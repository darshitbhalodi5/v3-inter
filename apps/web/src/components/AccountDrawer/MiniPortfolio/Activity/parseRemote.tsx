...
function isSpam(
  ...
  if (
    !SPAMMABLE_ACTIVITY_TYPES.includes(details.type) ||
    details.from === account
  )
    return false;
  return (
    NftTransfer.some((nft) => nft.asset.isSpam) ||
    TokenTransfer.some((t) => t.asset.project?.isSpam)
  );
}
...
export function parseSwapAmounts(
  ...
  const refund = changes.TokenTransfer.find(
    (t) =>
      ...
      t.asset.standard === NATIVE_CHAIN_ID,
  );
  ...
  const inputCurrencyId =
    sent.asset.standard === NATIVE_CHAIN_ID ? "ETH" : sent.asset.address;
  ...
  const refundQuantity = refund
    ...
    : BigNumber.from(0);
  const receivedQuantity = parseUnits(
    received.quantity,
...
function parseLend(
  ...
  const native = changes.TokenTransfer.find(
    (t) => t.tokenStandard === NATIVE_CHAIN_ID,
  )?.asset;
...
export function offchainOrderDetailsFromGraphQLTransactionActivity(
  ...
  const { inputCurrencyId, outputCurrencyId, inputAmountRaw, outputAmountRaw } =
    swapAmounts;
...
function swapOrderTypeToSignatureType(
...
function parseUniswapXOrder({
  details,
  chain,
  timestamp,
}: OrderActivity): Activity | undefined {
  ...
  return {
    ...
    offchainOrderDetails: {
      ...
      swapInfo: {
        ...
        inputCurrencyAmountRaw: parseUnits(
          inputTokenQuantity,
          inputToken.decimals,
        ).toString(),
        expectedOutputCurrencyAmountRaw: parseUnits(
          outputTokenQuantity,
          outputToken.decimals,
        ).toString(),
        minimumOutputCurrencyAmountRaw: parseUnits(
          outputTokenQuantity,
          outputToken.decimals,
        ).toString(),
        settledOutputCurrencyAmountRaw: parseUnits(
          outputTokenQuantity,
          outputToken.decimals,
        ).toString(),
      },
  ...
  try {
    ...
    if (!supportedChain) {
      logSentryErrorForUnsupportedChain({
        ...
        errorMessage:
          "Invalid activity from unsupported chain received from GQL",
      });
      return undefined;
    }
