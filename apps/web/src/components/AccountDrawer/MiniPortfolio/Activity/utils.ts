...
export async function cancelMultipleUniswapXOrders({
  ...
  try {
    ...
    for (const params of cancelParams) {
      const tx = await permit2.invalidateUnorderedNonces(
        params.word,
        params.mask,
      );
...
async function getCancelMultipleUniswapXOrdersTransaction(
  ...
  try {
    const tx = await permit2.populateTransaction.invalidateUnorderedNonces(
      cancelParams[0].word,
      cancelParams[0].mask,
    );
    return {
...
export function useCreateCancelTransactionRequest(
  ...
  const transactionFetcher = useCallback(() => {
    if (
      !params ||
      !params.orders ||
      params.orders.filter(({ encodedOrder }) => Boolean(encodedOrder))
        .length === 0 ||
      !permit2
    )
      return;
