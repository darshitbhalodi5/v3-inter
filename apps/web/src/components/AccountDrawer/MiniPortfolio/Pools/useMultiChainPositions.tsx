...
import { Pool, Position, computePoolAddress } from "darshitswap-v3-sdk";
...
function createPositionInfo(
  ...
  const inRange =
    slot0.tick >= details.tickLower && slot0.tick < details.tickUpper;
...
export default function useMultiChainPositions(
  ...
  const positionsLoading =
    !cachedPositions?.result && positionsFetching.current;
    ...
    async (
      ...
      const fees = (await pm.callStatic.multicall(callData)).reduce(
        (acc, feeBytes, index) => {
          ...
          acc[key] = pm.interface.decodeFunctionResult(
            "collect",
            feeBytes,
          ) as FeeAmounts;
          return acc;
  ...
  const fetchPositionIds = useCallback(
    async (pm: NonfungiblePositionManager, balance: BigNumber) => {
      ...
      return (await pm.callStatic.multicall(callData)).map((idByte) =>
        BigNumber.from(idByte),
      );
    },
    [account],
  );
  ...
  const fetchPositionDetails = useCallback(
    async (pm: NonfungiblePositionManager, positionIds: BigNumber[]) => {
      const callData = positionIds.map((id) =>
        pm.interface.encodeFunctionData("positions", [id]),
      );
  ...
  const fetchPositionInfo = useCallback(
    async (
      ...
      positionDetails.forEach((details) => {
        const tokenA =
          tokens[details.token0] ??
          new Token(chainId, details.token0, DEFAULT_ERC20_DECIMALS);
        const tokenB =
  ...
  const fetchAllPositions = useCallback(async () => {
    ...
    const positions = (
      await Promise.all(chains.map(fetchPositionsForChain))
    ).flat();
          ...
          ? [
              ...
              parseFloat(
                CurrencyAmount.fromRawAmount(
                  ...
                  feeMap[key]?.[0].toString(),
                ).toExact(),
              ),
              parseFloat(
                CurrencyAmount.fromRawAmount(
                  position.pool.token1,
                  feeMap[key]?.[1].toString(),
                ).toExact(),
              ),
            ]
          : undefined;
        const prices = [
          priceMap[currencyKey(position.pool.token0)],
          priceMap[currencyKey(position.pool.token1)],
        ];
