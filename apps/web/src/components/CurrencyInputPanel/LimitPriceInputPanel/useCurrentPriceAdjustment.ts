import { Currency, CurrencyAmount, Fraction, Price } from "darshitswap-sdk-core";
...
export function useCurrentPriceAdjustment({
  ...
  return useMemo(() => {
    ...
    const scaledMarketQuote = JSBI.BigInt(
      marketQuote.multiply(decimalsScalar).toFixed(0),
    );
    const scaledPriceQuote = JSBI.BigInt(
      parsedPriceQuote.multiply(decimalsScalar).toFixed(0),
    );
    ...
    const currentPriceAdjustment = Math.round(
      Number(percentageChange.multiply(100).toFixed(2)),
    );

    const priceBelowMarket = limitPriceInverted
      ? currentPriceAdjustment > 0
      : currentPriceAdjustment < 0;

    return {
      currentPriceAdjustment,
      priceError: priceBelowMarket
        ? LimitPriceErrorType.BELOW_MARKET
        : undefined,
    };
  }, [
    parsedLimitPrice,
