...
import { ChainId, Token } from "darshitswap-sdk-core";
import {
  PoolTableColumns,
  PoolsTable,
  sortAscendingAtom,
  sortMethodAtom,
} from "components/Pools/PoolTable/PoolTable";
...
export function TokenDetailsPoolsTable({
  ...
  const { pools, loading, errorV2, errorV3, loadMore } =
    usePoolsFromTokenAddress(referenceToken.address, sortState, chainId);
  const combinedError =
    errorV2 && errorV3
      ? new ApolloError({
  ...
  return (
    <div
      data-testid={`tdp-pools-table-${referenceToken.address.toLowerCase()}`}
    >
      <PoolsTable
        pools={pools}
