import { Fraction, TradeType } from "darshitswap-sdk-core";
...
function MigrateLiquidityToV3Summary({
  ...
  return (
    <Trans>
      Migrate {{ baseSymbol: baseCurrency?.symbol }}/
      {{ quoteSymbol: quoteCurrency?.symbol }} liquidity to V3
    </Trans>
  );
}
...
function CreateV3PoolSummary({
  ...
  return (
    <Trans>
      Create {{ baseSymbol: baseCurrency?.symbol }}/
      {{ quoteSymbol: quoteCurrency?.symbol }} V3 pool
    </Trans>
  );
}
...
function CollectFeesSummary({
  ...
  return (
    <Trans>
      Collect {{ symbol0: currency0?.symbol }}/{{ symbol1: currency1?.symbol }}{" "}
      fees
    </Trans>
  );
}
