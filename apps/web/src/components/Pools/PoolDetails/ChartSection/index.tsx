import { ChainId, CurrencyAmount, Token } from "darshitswap-sdk-core";
import { FeeAmount } from "darshitswap-v3-sdk";
import { ChartHeader } from "components/Charts/ChartHeader";
import { Chart } from "components/Charts/ChartModel";
import {
  LiquidityBarChartModel,
  useLiquidityBarData,
} from "components/Charts/LiquidityChart";
...
const PDP_CHART_SELECTOR_OPTIONS = [
  ChartType.VOLUME,
  ChartType.PRICE,
  ChartType.LIQUIDITY,
] as const;
...
function usePDPChartState(
  ...
  const [chartType, setChartType] = useState<PoolsDetailsChartType>(
    ChartType.VOLUME,
  );
...
export default function ChartSection(props: ChartSectionProps) {
  ...
  const { setChartType, timePeriod, setTimePeriod, activeQuery } =
    usePDPChartState(
      props.poolData,
      currencyA?.wrapped,
      currencyB?.wrapped,
  ...
  const ChartBody = (() => {
    ...
    if (
      activeQuery.dataQuality === DataQuality.INVALID ||
      !currencyA ||
      !currencyB
    ) {
      const errorText = loading ? undefined : (
        <Trans>Unable to display historical data for the current pool.</Trans>
      );
      return (
        <ChartSkeleton
          type={activeQuery.chartType}
          ...
          <TimePeriodSelectorContainer>
            <PillMultiToggle
              ...
              onSelectOption={(option) => {
                const time = getTimePeriodFromDisplay(
                  option as TimePeriodDisplay,
                );
                if (time === timePeriod) {
                  refitChartContent?.();
                } else {
...
function PriceChart({
  ...
  const [primaryToken, referenceToken] = isReversed
    ? [tokenB, tokenA]
    : [tokenA, tokenB];

  const params = useMemo(
    () => ({ data, stale, type: PriceChartType.LINE }),
    [data, stale],
  );
    ...
    <StyledChart Model={PriceChartModel} params={params}>
      {(crosshairData) => {
        ...
        const currencyBAmountRaw = Math.floor(
          (displayValue.value ?? displayValue.close) *
            10 ** referenceToken.decimals,
        );
        const priceDisplay = (
          <PriceDisplayContainer>
            ...
            <ChartPriceText color="neutral2">
              {stablecoinPrice
                ? "(" + formatPrice({ price: stablecoinPrice }) + ")"
                : ""}
            </ChartPriceText>
          </PriceDisplayContainer>
        );
        return (
          <ChartHeader
            ...
            additionalFields={
              <PriceChartDelta
                startingPrice={data[0]}
                endingPrice={displayValue}
              />
            }
