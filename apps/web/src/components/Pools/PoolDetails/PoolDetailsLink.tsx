...
export function PoolDetailsLink({
  address,
  chainId,
  tokens,
  loading,
}: PoolDetailsLinkProps) {
  ...
  const handleTokenTextClick = useCallback(() => {
    if (!isPool) {
      navigate(
        getTokenDetailsURL({ address: tokens[0]?.address, chain: chainName }),
      );
    }
  ...
  return (
    <Row align="space-between">
      <TokenTextWrapper
        data-testid={
          isPool
            ? `pdp-pool-logo-${tokens[0]?.symbol}-${tokens[1]?.symbol}`
            : `pdp-token-logo-${tokens[0]?.symbol}`
        }
        isClickable={!isPool}
        ...
        {!isNative && (
          ...
          >
            <CopyAddress data-testid={`copy-address-${address}`} onClick={copy}>
              {shortenAddress(
                address,
                truncateAddress ? 2 : undefined,
                truncateAddress === "both" ? 2 : undefined,
              )}
              <StyledCopyIcon />
            </CopyAddress>
          </Tooltip>
