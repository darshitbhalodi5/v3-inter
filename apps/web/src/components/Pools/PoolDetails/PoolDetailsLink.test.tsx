...
describe("PoolDetailsHeader", () => {
  ...
  it("renders link for pool address", async () => {
    ...
    expect(
      screen.getByTestId(`copy-address-${usdcWethPoolAddress}`),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId(
        `explorer-url-https://etherscan.io/address/${usdcWethPoolAddress}`,
      ),
    ).toBeInTheDocument();
  });
  ...
  it("renders link for token address", async () => {
    ...
    expect(
      screen.getByTestId(`copy-address-${USDC_MAINNET.address}`),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId(
        `explorer-url-https://etherscan.io/token/${USDC_MAINNET.address}`,
      ),
    ).toBeInTheDocument();
  });
});
