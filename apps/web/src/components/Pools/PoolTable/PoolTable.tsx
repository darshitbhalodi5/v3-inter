...
import { ChainId, Percent } from "darshitswap-sdk-core";
...
const HEADER_DESCRIPTIONS: Record<PoolSortFields, ReactNode | undefined> = {
  ...
  [PoolSortFields.OneDayApr]: (
    <Trans>
      1 day APR refers to the amount of trading fees relative to total value
      locked (TVL) within a pool. 1 day APR = 24H Fees / TVL
    </Trans>
  ),
};
  ...
  return (
    <Row gap="sm">
      ...
      {protocolVersion === ProtocolVersion.V2 && (
        <Badge>{protocolVersion.toLowerCase()}</Badge>
      )}
  ...
  return (
    ...
    >
      <ClickableHeaderRow $justify="flex-end" onClick={handleSortCategory}>
        ...
        <HeaderSortText $active={isCurrentSortMethod}>
...
export function TopPoolTable() {
  ...
  const combinedError =
    errorV2 && errorV3
      ? new ApolloError({
          errorMessage: `Could not retrieve V2 and V3 Top Pools on chain: ${chainId}`,
        })
      : undefined;
...
export function PoolsTable({
  ...
  const sortAscending = useAtomValue(sortAscendingAtom);
  const orderDirection = sortAscending
    ? OrderDirection.Asc
    : OrderDirection.Desc;
  const sortMethod = useAtomValue(sortMethodAtom);
