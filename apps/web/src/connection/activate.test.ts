...
function createMockConnection(
  ...
  return {
    getProviderInfo: () => ({ name: "Test Connection" }),
...
it("Should call activate function on a connection", async () => {
  ...
  act(() => {
    activationCall = result.current.tryActivation(
      mockConnection,
      onSuccess,
      ChainId.OPTIMISM,
    );
  ...
  expect(mockConnection.overrideActivate).toHaveBeenCalledWith(
    ChainId.OPTIMISM,
  );
  expect(mockConnection.connector.activate).toHaveBeenCalledTimes(1);
  expect(console.debug).toHaveBeenLastCalledWith(
    `Connection activating: ${mockConnection.getProviderInfo().name}`,
  );
  ...
  expect(mockConnection.connector.activate).toHaveBeenCalledTimes(1);
  expect(console.debug).toHaveBeenLastCalledWith(
    `Connection activated: ${mockConnection.getProviderInfo().name}`,
  );
...
it("Should properly deactivate pending connection attempts", async () => {
  ...
  expect(console.debug).not.toHaveBeenLastCalledWith(
    `Connection activated: ${mockConnection.getProviderInfo().name}`,
  );
  expect(onSuccess).toHaveBeenCalledTimes(0);
});
...
it("Should properly display error state", async () => {
  ...
  expect(console.debug).toHaveBeenLastCalledWith(
    `Connection failed: ${mockConnection.getProviderInfo().name}`,
  );
  expect(console.debug).toHaveBeenCalledTimes(2);
  expect(mockConnection.connector.activate).toHaveBeenCalledTimes(1);
  expect(onSuccess).toHaveBeenCalledTimes(0);
    ...
    const injectedConection = createMockConnection(
      jest
        ...
        .mockImplementationOnce(() =>
          Promise.reject({ code: ErrorCode.USER_REJECTED_REQUEST }),
        )
        .mockImplementationOnce(() => Promise.resolve),
      jest.fn(),
  ...
  it("handles Coinbase user-rejection error", async () => {
    ...
    const coinbaseConnection = createMockConnection(
      jest
        .fn()
        .mockImplementationOnce(() =>
          Promise.reject(ErrorCode.CB_REJECTED_REQUEST),
        )
        .mockImplementationOnce(() => Promise.resolve),
      jest.fn(),
    ...
    await act(() =>
      result.current.tryActivation(coinbaseConnection, onSuccess),
    );
  ...
  it("handles WalletConect Modal close error", async () => {
    ...
    const wcConnection = createMockConnection(
      jest
        .fn()
        .mockImplementationOnce(() =>
          Promise.reject(ErrorCode.WC_V2_MODAL_CLOSED),
        )
        .mockImplementationOnce(() => Promise.resolve),
      jest.fn(),
