...
import { CurrencyAmount, WETH9 } from "darshitswap-sdk-core";
import { FeeAmount, Pool } from "darshitswap-v3-sdk";
...
const positionDetails: PositionDetails = {
  ...
  feeGrowthInside0LastX128: BigNumber.from(
    "0xfffffffffffffffffffffffffffffffffffff14da6c0d4c3e0f2ff473efb5278",
  ),
  feeGrowthInside1LastX128: BigNumber.from(
    "0xfffffffffffffffffffffffffffffd7e342f500425d9f6c176349d1ef4621e0d",
  ),
...
const WETH_AMOUNT = CurrencyAmount.fromRawAmount(
  WETH9[1],
  "500807669662847869",
);
...
describe("position page", () => {
  it("correctly collects the correct amount", () => {
    ...
    const { formatCurrencyAmount } = renderHook(() => useFormatter()).result
      .current;
    render(<PositionPage />);

    const collectFeesButton = screen.queryByTestId(
      "collect-fees-button",
    ) as HTMLButtonElement;
    expect(collectFeesButton).toBeInTheDocument();
    expect(screen.getByText("Collect fees")).toBeInTheDocument();
    expect(
      screen.getByText(formatCurrencyAmount({ amount: USDC_AMOUNT })),
    ).toBeInTheDocument();
    expect(
      screen.getByText(formatCurrencyAmount({ amount: WETH_AMOUNT })),
    ).toBeInTheDocument();
    ...
    const modalCollectFeesButton = screen.queryByTestId(
