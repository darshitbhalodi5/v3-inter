...
import { NonfungiblePositionManager } from "darshitswap-v3-sdk";
...
export default function RemoveLiquidityV3() {
  ...
  const { position, loading } = useV3PositionFromTokenId(
...
function Remove({ tokenId }: { tokenId: BigNumber }) {
  ...
  const pendingText = (
    <Trans>
      Removing {{ amt: liquidityValue0?.toSignificant(6) }}{" "}
      {{ sym: liquidityValue0?.currency?.symbol }} and{" "}
      {{ amt2: liquidityValue1?.toSignificant(6) }}{" "}
      {{ sym2: liquidityValue1?.currency?.symbol }}
    </Trans>
  );
      ...
      <AutoColumn gap="sm" style={{ padding: "16px" }}>
        <RowBetween align="flex-end">
          ...
          <RowFixed>
            <Text fontSize={16} fontWeight={535} marginLeft="6px">
              {liquidityValue0 &&
                formatCurrencyAmount({ amount: liquidityValue0 })}
            </Text>
            <CurrencyLogo
              size="20px"
        ...
        <RowBetween align="flex-end">
          ...
          <RowFixed>
            <Text fontSize={16} fontWeight={535} marginLeft="6px">
              {liquidityValue1 &&
                formatCurrencyAmount({ amount: liquidityValue1 })}
            </Text>
            <CurrencyLogo
              size="20px"
              style={{ marginLeft: "8px" }}
        ...
        {feeValue0?.greaterThan(0) || feeValue1?.greaterThan(0) ? (
          <>
            ...
            <RowBetween>
              <Text fontSize={16} fontWeight={535}>
                <Trans>
                  {{ sym: feeValue0?.currency?.symbol }} Fees Earned:
                </Trans>
              </Text>
              <RowFixed>
                <Text fontSize={16} fontWeight={535} marginLeft="6px">
            ...
            <RowBetween>
              <Text fontSize={16} fontWeight={535}>
                <Trans>
                  {{ sym: feeValue1?.currency?.symbol }} Fees Earned:
                </Trans>
              </Text>
              <RowFixed>
                <Text fontSize={16} fontWeight={535} marginLeft="6px">
  ...
  const showCollectAsWeth = Boolean(
    liquidityValue0?.currency &&
      ...
      (liquidityValue0.currency.isNative ||
        ...
        WRAPPED_NATIVE_CURRENCY[liquidityValue0.currency.chainId]?.equals(
          liquidityValue0.currency.wrapped,
        ) ||
        WRAPPED_NATIVE_CURRENCY[liquidityValue1.currency.chainId]?.equals(
          liquidityValue1.currency.wrapped,
        )),
  );
  return (
    <AutoColumn>
              ...
              <LightCard>
                <AutoColumn gap="md">
                  <RowBetween>
                    ...
                    >
                      <Trans>
                        Pooled {{ symbol: liquidityValue0?.currency?.symbol }}:
                      </Trans>
                    </Text>
                    <RowFixed>
                      <Text fontSize={16} fontWeight={535} marginLeft="6px">
                  ...
                  <RowBetween>
                    ...
                    >
                      <Trans>
                        Pooled {{ symbol: liquidityValue1?.currency?.symbol }}:
                      </Trans>
                    </Text>
                    <RowFixed>
                      <Text fontSize={16} fontWeight={535} marginLeft="6px">
                  ...
                  {feeValue0?.greaterThan(0) || feeValue1?.greaterThan(0) ? (
                    <>
                      ...
                      <RowBetween>
                        <Text fontSize={16} fontWeight={535}>
                          <Trans>
                            {{ symbol: feeValue0?.currency?.symbol }} Fees
                            Earned:
                          </Trans>
                        </Text>
                        <RowFixed>
                          <Text fontSize={16} fontWeight={535} marginLeft="6px">
                            {feeValue0 &&
                              formatCurrencyAmount({ amount: feeValue0 })}
                          </Text>
                          <CurrencyLogo
                            size="20px"
                            style={{ marginLeft: "8px" }}
                      ...
                      <RowBetween>
                        <Text fontSize={16} fontWeight={535}>
                          <Trans>
                            {{ symbol: feeValue1?.currency?.symbol }} Fees
                            Earned:
                          </Trans>
                        </Text>
                        <RowFixed>
                          <Text fontSize={16} fontWeight={535} marginLeft="6px">
                            {feeValue1 &&
                              formatCurrencyAmount({ amount: feeValue1 })}
                          </Text>
                          <CurrencyLogo
                            size="20px"
                            style={{ marginLeft: "8px" }}
