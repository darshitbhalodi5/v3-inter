...
function getTitle({
  ...
  if (priceError === LimitPriceErrorType.CALCULATION_ERROR) {
    ...
    return (
      <Trans>
        Buying {{ symbol: outputCurrency.symbol }} above market price
      </Trans>
    );
  } else {
    return (
      <Trans>
        Selling {{ symbol: inputCurrency.symbol }} below market price
      </Trans>
    );
  }
}
...
function getDescription({
  ...
  if (priceError === LimitPriceErrorType.CALCULATION_ERROR) {
    return (
      <Trans>
        We are unable to calculate the current market price. To avoid submitting
        an order below market price, please check your network connection and
        try again.
      </Trans>
    );
    ...
    return (
      <Trans>
        Your limit price is {{ pct: Math.abs(priceAdjustmentPercentage) }}%
        higher than market. Adjust your limit price to proceed.
      </Trans>
    );
    ...
    return (
      <Trans>
        Your limit price is {{ pct: Math.abs(priceAdjustmentPercentage) }}%
        lower than market. Adjust your limit price to proceed.
      </Trans>
    );
  }
  return null;
