import { ChainId, Currency } from "darshitswap-sdk-core";
...
import {
  ...
  useActiveLocalCurrencyComponents,
} from "hooks/useActiveLocalCurrency";
...
const StyledNumericalInput = styled(NumericalInput)<{ $width?: number }>`
  ...
  width: ${({ $width }) =>
    `${
      $width ?? 43
    }px`}; // this value is from the size of a 0 which is the default value
...
const AlternateCurrencyDisplay = ({
  ...
  const { formatConvertedFiatNumberOrString, formatNumberOrString } =
    useFormatter();
  ...
  const displayCurrency = inputInFiat
...
const InputErrorLookup = {
  ...
  [SendInputError.INSUFFICIENT_FUNDS_FOR_GAS]: (
    <Trans>Insufficient funds to cover network fee</Trans>
  ),
};
...
export default function SendCurrencyInputForm({
  ...
  const { inputInFiat, exactAmountToken, exactAmountFiat, inputCurrency } =
    sendState;
  const { currencyBalance, exactAmountOut, parsedTokenAmount } =
    derivedSendInfo;
  ...
  const [postWidthAdjustedDisplayValue, setPostWidthAdjustedDisplayValue] =
    useState("");
      ...
      <InputWrapper>
        ...
        <NumericalInputWrapper>
          ...
          <StyledNumericalInput
            ...
            placeholder="0"
            $width={
              displayValue && hiddenObserver.width
                ? hiddenObserver.width + 1
                : undefined
            }
            maxDecimals={inputInFiat ? 6 : inputCurrency?.decimals}
          />
          <NumericalInputMimic ref={hiddenObserver.ref}>
            {displayValue}
          </NumericalInputMimic>
        </NumericalInputWrapper>
