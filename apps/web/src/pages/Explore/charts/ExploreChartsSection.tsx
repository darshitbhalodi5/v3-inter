...
} from "graphql/data/protocolStats";
import {
  TimePeriod,
      ...
      {(() => {
        if (dataQuality === DataQuality.INVALID) {
          const errorText = loading ? undefined : (
            <Trans>
              Unable to display historical volume data for the current chain.
            </Trans>
          );
          return (
            <ChartSkeleton
          ...
          >
            {(crosshairData) => (
              <ChartHeader
                value={
                  crosshairData
                    ? getCumulativeSum(crosshairData)
                    : getCumulativeVolume(entries)
                }
    ...
    <SectionContainer>
      ...
      {(() => {
        if (dataQuality === DataQuality.INVALID) {
          const errorText = loading ? undefined : (
            <Trans>
              Unable to display historical TVL data for the current chain.
            </Trans>
          );
          return (
            <ChartSkeleton
