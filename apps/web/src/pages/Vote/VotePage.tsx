...
import { CurrencyAmount, Fraction, Token } from "darshitswap-sdk-core";
...
const Progress = styled.div<{
  ...
  background-color: ${({ theme, status }) =>
    status === "for" ? theme.success : theme.critical};
...
export default function VotePage() {
  ...
  const eta = proposalData?.eta
    ? new Date(proposalData.eta.mul(ms(`1s`)).toNumber())
    : undefined;

  // get total votes and format percentages for UI
  const totalVotes = proposalData?.forCount?.add(proposalData.againstCount);
  const forPercentage = totalVotes
    ? proposalData?.forCount?.asFraction
        ?.divide(totalVotes.asFraction)
        ?.multiply(100)
    : undefined;
  const againstPercentage = forPercentage
    ? new Fraction(100).subtract(forPercentage)
    : undefined;
  ...
  const showExecuteButton =
    account && proposalData?.status === ProposalState.QUEUED;
                  ...
                  <GrayCard>
                    <ThemedText.DeprecatedBlack>
                      <Trans>
                        ...
                        another address before block{" "}
                        {{ startBlock: proposalData.startBlock }} are eligible
                        for voting.
                      </Trans>{" "}
                      {showLinkForUnlock && (
                        <span>
              ...
              <>
                {eta && (
                  <RowBetween>
                    <ThemedText.DeprecatedBlack>
                      <Trans>
                        This proposal may be executed after{" "}
                ...
                <RowFixed style={{ width: "100%", gap: "12px" }}>
                  <ButtonPrimary
                    ...
                    disabled={
                      !currentTimestamp ||
                      !proposalData?.eta ||
                      currentTimestamp.lt(proposalData.eta)
                    }
                  >
                    <Trans>Execute</Trans>
                  </ButtonPrimary>
                ...
                <CardSection>
                  ...
                  <ProgressWrapper>
                    <Progress
                      status="for"
                      percentageString={
                        proposalData?.forCount.greaterThan(0)
                          ? `${forPercentage?.toFixed(0) ?? 0}%`
                          : "0%"
                      }
                    />
                  </ProgressWrapper>
                ...
                <CardSection>
                  ...
                  <ProgressWrapper>
                    <Progress
                      status="against"
                      percentageString={
                        proposalData?.againstCount?.greaterThan(0)
                          ? `${againstPercentage?.toFixed(0) ?? 0}%`
                          : "0%"
                      }
                    />
                  </ProgressWrapper>
