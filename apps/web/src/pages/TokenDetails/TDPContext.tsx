...
export type MultiChainMap = {
  [chain in Chain]?:
    | { address?: string; balance?: PortfolioTokenBalancePartsFragment }
    | undefined;
};
...
export function TDPProvider({
  children,
  contextValue,
}: PropsWithChildren<{ contextValue: LoadedTDPContext }>) {
  return (
    <TDPContext.Provider value={contextValue}>{children}</TDPContext.Provider>
  );
}
