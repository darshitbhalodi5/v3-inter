...
function useTDPCurrency(
  ...
  const skipOnChainFetch =
    Boolean(queryCurrency) || currencyChainId !== appChainId;
  ...
  return useMemo(() => {
    ...
    return tokensAcrossChains.reduce<MultiChainMap>((map, current) => {
      if (current) {
        ...
        update.balance = tokenBalances?.find(
          (tokenBalance) => tokenBalance?.token?.id === current.id,
        );
...
function useCreateTDPContext(): PendingTDPContext | LoadedTDPContext {
  ...
  const { preloadedLogoSrc } =
    (useLocation().state as { preloadedLogoSrc?: string }) ?? {};
  const extractedColorSrc =
    tokenQuery.data?.token?.project?.logoUrl ?? preloadedLogoSrc;
  const tokenColor =
    useSrcColor(extractedColorSrc, tokenQuery.data?.token?.name, theme.surface2)
