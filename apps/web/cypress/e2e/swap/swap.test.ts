  ...
  describe("Swap on main page", () => {
    ...
    it("swaps ETH for USDC", () => {
      cy.interceptGraphqlOperation(
        "Activity",
        "mini-portfolio/empty_activity.json",
      );
      cy.visit("/swap");
      ...
      getBalance(USDC_MAINNET).then((initialBalance) => {
        ...
        cy.get(getTestSelector("token-search-input")).type(
        ...
        // Verify logging
        cy.waitForAmplitudeEvent(SwapEventName.SWAP_QUOTE_RECEIVED).then(
          (event: any) => {
            cy.wrap(event.event_properties).should(
              "have.property",
              "quote_latency_milliseconds",
            );
            cy.wrap(event.event_properties.quote_latency_milliseconds).should(
              "be.a",
              "number",
            );
            cy.wrap(event.event_properties.quote_latency_milliseconds).should(
        ...
        cy.wait("@eth_estimateGas")
          .wait("@eth_sendRawTransaction")
          .wait("@eth_getTransactionReceipt");
