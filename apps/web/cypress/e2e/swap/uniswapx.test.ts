import { ChainId, CurrencyAmount } from "darshitswap-sdk-core";
...
const OrderSubmissionEndpoint =
  "https://interface.gateway.uniswap.org/v2/order";
  ...
  cy.intercept(QuoteEndpoint, (req: CyHttpMessages.IncomingHttpRequest) => {
    ...
    if (body.intent === "pricing") {
      const pricingFixture =
        body.tokenIn === USDC_MAINNET.address
          ? PricingQuoteUSDC
          : PricingQuoteDAI;
...
function stubSwapTxReceipt() {
  cy.hardhat().then((hardhat) => {
    cy.fixture("uniswapx/fillTransactionReceipt.json").then((mockTxReceipt) => {
      const getTransactionReceiptStub = cy
        .stub(hardhat.provider, "getTransactionReceipt")
        .log(false);
      getTransactionReceiptStub
        .withArgs(mockTxReceipt.transactionHash)
...
describe("UniswapX Toggle", () => {
  beforeEach(() => {
    ...
    cy.visit(
      `/swap/?inputCurrency=${USDC_MAINNET.address}&outputCurrency=${DAI.address}`,
    );
  });
...
describe("UniswapX Orders", () => {
  beforeEach(() => {
    stubNonPriceQuoteWith(QuoteWhereUniswapXIsBetter);
    cy.intercept(OrderSubmissionEndpoint, {
      fixture: "uniswapx/orderResponse.json",
    });
    cy.intercept(OrderStatusEndpoint, {
      fixture: "uniswapx/openStatusResponse.json",
    });
    ...
    cy.hardhat().then(async (hardhat) => {
      await hardhat.fund(
        hardhat.wallet,
        CurrencyAmount.fromRawAmount(USDC_MAINNET, 3e8),
      );
      await hardhat.approval.setPermit2Allowance({
        owner: hardhat.wallet,
        token: USDC_MAINNET,
      });
      await hardhat.approval.setTokenAllowanceForPermit2({
        owner: hardhat.wallet,
        token: USDC_MAINNET,
      });
    });
  ...
  it("can swap exact-in trades using uniswapX", () => {
    ...
    cy.intercept(OrderStatusEndpoint, {
      fixture: "uniswapx/filledStatusResponse.json",
    });
  ...
  it("renders proper view if uniswapx order expires", () => {
    ...
    cy.intercept(OrderStatusEndpoint, {
      fixture: "uniswapx/expiredStatusResponse.json",
    });
  ...
  it("renders proper view if uniswapx order has insufficient funds", () => {
    ...
    cy.intercept(OrderStatusEndpoint, {
      fixture: "uniswapx/insufficientFundsStatusResponse.json",
    });
  ...
  it("cancels a pending uniswapx order", () => {
    ...
    cy.get(getTestSelector("mini-portfolio-navbar"))
      .contains("Activity")
      .click();

    // Open pending order modal
    cy.contains("Swapping").click();
    cy.get(getTestSelector("offchain-activity-modal")).contains(
      "Transaction details",
    );

    // Cancel order
    cy.get(getTestSelector("offchain-activity-modal"))
      .contains("Cancel")
      .click();
    ...
    cy.intercept(OrderStatusEndpoint, {
      fixture: "uniswapx/cancelledStatusResponse.json",
    });
...
describe("UniswapX Eth Input", () => {
  beforeEach(() => {
    ...
    cy.hardhat({ automine: false }).then(async (hardhat) => {
      await hardhat.fund(
        hardhat.wallet,
        CurrencyAmount.fromRawAmount(nativeOnChain(ChainId.MAINNET), 2e18),
      );
      await hardhat.mine();
    });
...
describe("UniswapX activity history", () => {
  ...
  it("deduplicates remote vs local uniswapx orders", () => {
    ...
    cy.fixture("mini-portfolio/uniswapx_activity.json").then(
      (uniswapXActivity) => {
        // Replace fixture's timestamp with current time
        uniswapXActivity.data.portfolios[0].assetActivities[0].timestamp =
          Date.now() / 1000;
    ...
    cy.get(getTestSelector("activity-content"))
      .contains("Swapped")
