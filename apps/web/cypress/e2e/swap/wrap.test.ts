import { ChainId, CurrencyAmount, WETH9 } from "darshitswap-sdk-core";
...
describe("Swap wrap", () => {
  beforeEach(() => {
    cy.visit(`/swap?inputCurrency=ETH&outputCurrency=${WETH.address}`).hardhat({
      automine: false,
    });
  });
  ...
  it("should be able to unwrap WETH", () => {
    cy.hardhat().then(async (hardhat) => {
      await hardhat.fund(
        hardhat.wallet,
        CurrencyAmount.fromRawAmount(WETH, 1e18),
      );
      await hardhat.mine();
    });
