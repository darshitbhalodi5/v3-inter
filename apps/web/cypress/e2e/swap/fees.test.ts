import { CurrencyAmount } from "darshitswap-sdk-core";
...
describe("Swap with fees", () => {
      ...
      cy.hardhat().then((hardhat) => {
        getBalance(USDC_MAINNET).then((initialBalance) => {
          ...
          cy.wait("@quoteFetch")
            .its("response.body")
            .then(
              ({ quote: { portionBips, portionRecipient, portionAmount } }) => {
                ...
                cy.then(() =>
                  hardhat.getBalance(portionRecipient, USDC_MAINNET),
                ).then((initialRecipientBalance) => {
                  ...
                  cy.get("#swap-currency-output").contains(
                  ...
                  cy.then(() =>
                    hardhat.getBalance(portionRecipient, USDC_MAINNET),
                  ).then((finalRecipientBalance) => {
                    const expectedFinalRecipientBalance =
                      initialRecipientBalance.add(feeCurrencyAmount);
                    cy.then(() =>
                      finalRecipientBalance.equalTo(
                        expectedFinalRecipientBalance,
                      ),
                    ).should("be.true");
                  });
                });
          ...
          .then(({ quote: { portionBips, portionRecipient } }) => {
            ...
            cy.then(() =>
              ...
              cy.then(() =>
                ...
                cy.then(() =>
                  finalRecipientBalance.greaterThan(initialRecipientBalance),
                ).should("be.true");
              });
            });
          });
  ...
  describe("UniswapX swaps", () => {
    it("displays UniswapX fee in UI", () => {
      ...
      cy.fixture("uniswapx/feeQuote.json").then(
        (fixture: URADutchOrderQuoteResponse) => {
