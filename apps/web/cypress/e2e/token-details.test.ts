...
describe("Token details", () => {
  ...
  it("UNI token should have all information populated", () => {
    ...
    cy.get(getTestSelector("token-details-info-links")).within(() => {
      cy.contains("Etherscan")
        ...
        .and("include", `etherscan.io/token/${UNI_ADDRESS}`);
      cy.contains("Website")
        .should("have.attr", "href")
        .and("include", "uniswap.org");
      cy.contains("Twitter")
        .should("have.attr", "href")
        .and("include", "x.com/Uniswap");
    });
  ...
  it("token with warning and low trading volume should have all information populated", () => {
    ...
    cy.visit(
      "/explore/tokens/ethereum/0x1eFBB78C8b917f67986BcE54cE575069c0143681",
    );
    ...
    cy.get(getTestSelector("token-details-info-links")).within(() => {
      cy.contains("Etherscan")
        ...
        .and(
          "include",
          "etherscan.io/token/0x1eFBB78C8b917f67986BcE54cE575069c0143681",
        );
    });

    // Contract address should be displayed
    cy.contains(
      shortenAddress("0x1eFBB78C8b917f67986BcE54cE575069c0143681"),
    ).should("exist");
  ...
  describe("swapping", () => {
    ...
    it("should have the expected output for a tokens detail page", () => {
      cy.get(`#swap-currency-input .token-amount-input`).should(
        "have.value",
        "",
      );
    ...
    it("should show a L2 token even if the user is connected to a different network", () => {
      ...
      cy.get(getTestSelector("tokens-network-filter-selected")).should(
        "contain",
        "Arbitrum",
      );
      cy.get(
        getTestSelector(`token-table-row-${ARB.address.toLowerCase()}`),
      ).click();
      cy.get(`#swap-currency-output .token-symbol-container`).should(
        "contain.text",
        "ARB",
      );
      cy.get(getTestSelector("open-settings-dialog-button")).should(
        "be.disabled",
      );
