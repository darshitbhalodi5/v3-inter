...
import { MaxUint160, MaxUint256 } from "darshitswap-permit2-sdk";
import { CurrencyAmount, Token } from "darshitswap-sdk-core";
...
describe("Permit2", () => {
  function setupInputs(inputToken: Token, outputToken: Token) {
    ...
    cy.visit(
      `/swap/?inputCurrency=${inputToken.address}&outputCurrency=${outputToken.address}`,
    );
  ...
  function expectTokenAllowanceForPermit2ToBeMax(inputToken: Token) {
    ...
    cy.hardhat()
      .then(({ approval, wallet }) =>
        approval.getTokenAllowanceForPermit2({
          owner: wallet,
          token: inputToken,
        }),
      )
  ...
  function expectPermit2AllowanceForUniversalRouterToBeMax(inputToken: Token) {
    cy.hardhat()
      .then(({ approval, wallet }) =>
        approval.getPermit2Allowance({ owner: wallet, token: inputToken }),
      )
      .then((allowance) => {
        Cypress.log({
          name: `Permit2 allowance: ${allowance.amount.toString()}`,
        });
  ...
  beforeEach(() =>
    cy.hardhat().then(async (hardhat) => {
      await hardhat.fund(
        hardhat.wallet,
        CurrencyAmount.fromRawAmount(DAI, 1e18),
      );
      await hardhat.mine();
    }),
  );
  ...
  describe("approval process (with intermediate screens)", () => {
    ...
    it("swaps USDT with existing permit, and existing but insufficient token approval", () => {
      cy.hardhat().then(async (hardhat) => {
        await hardhat.fund(
          hardhat.wallet,
          CurrencyAmount.fromRawAmount(USDT, 2e6),
        );
        await hardhat.mine();
        await hardhat.approval.setTokenAllowanceForPermit2(
          { owner: hardhat.wallet, token: USDT },
          1e6,
        );
        await hardhat.mine();
        await hardhat.approval.setPermit2Allowance({
      ...
      cy.hardhat()
        .then(({ approval, wallet }) =>
          approval.getTokenAllowanceForPermit2({ owner: wallet, token: USDT }),
        )
  ...
  it("swaps after handling user rejection of both approval and signature", () => {
    ...
    cy.hardhat().then((hardhat) => {
      ...
      const tokenApprovalStub = cy
        .stub(hardhat.wallet, "sendTransaction")
        .log(true);
      ...
      permitApprovalStub
        .withArgs("eth_signTypedData_v4")
        .rejects(USER_REJECTION); // rejects permit approval
      ...
      cy.wrap(permitApprovalStub).should(
        "be.calledWith",
  ...
  it("prompts signature when existing permit approval is expired", () => {
    ...
    cy.hardhat()
      .then(({ approval, wallet }) =>
        Promise.all([
          approval.setTokenAllowanceForPermit2({ owner: wallet, token: DAI }),
          approval.setPermit2Allowance(
            { owner: wallet, token: DAI },
            expiredAllowance,
          ),
        ]),
      )
