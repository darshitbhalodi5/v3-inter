...
fs.readFile("./public/tokens-sitemap.xml", "utf8", async (err, data) => {
  ...
  try {
    ...
    for (const chainName of chains) {
      ...
      const tokenAddresses = tokensJSON.data.topTokens.map((token) =>
...
fs.readFile("./public/nfts-sitemap.xml", "utf8", async (err, data) => {
  ...
  try {
    ...
    const collectionAddresses = nftJSON.data.topCollections.edges.map(
  ...
  try {
    ...
    for (const chainName of chains) {
      ...
      const v3PoolAddresses =
        poolsJSON.data.topV3Pools?.map((pool) => pool.address.toLowerCase()) ??
        [];
      const v2PoolAddresses =
        poolsJSON.data.topV2Pairs?.map((pool) => pool.address.toLowerCase()) ??
        [];
